...
sub test_generate_fetch_sql {
    ...
    $sql =~ s{\s+}{ }g;
    my $std_sql = 'SELECT *, round( extract( epoch from age( now(), timestamp ))) as age FROM tablename WHERE node_id = ? ORDER BY timestamp desc limit 4; ';
...
sub test_fetch_alert_data {
    ...
    my @std  = map { $SCHEMA->{$_} } keys %$SCHEMA;
    is_deeply( $records, \@std, 'fetch_alert_data() OK' );
    return;
}
