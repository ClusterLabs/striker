...
sub test_constructor {
    ...
    my $alert = AN::Alerts->new({ agents => { pid      => $PID,
					      program  => 'scanner',
...
#				  owner => $self,
    ...
    is_deeply( [ sort keys %$xlator ], [ 'agents', 'pid' ],
...
sub strings_table_loaded_pre_load {
    ...
    ok( ! $xlator->strings_table_loaded(),
...
sub load_strings_table {
    ...
    is_deeply( $xlator->language(), {'en_CA' => 'English (Canadian)',
			       'fr' => 'Français',
			       'ja' => '日本語'}, 'languages populated OK');
    is( ref $xlator->string(), 'HASH', 'string attribute is a hash');
    is( ref $xlator->string()->{lang}{en_CA}, 'HASH',
	'string en_CA sub-component is a hash');
    is_deeply( [sort keys %{$xlator->string->{lang}{en_CA}{key}}],
	       [ 'OLD_PROCESS_CRASH', 'OLD_PROCESS_RECENT_CRASH',
		 'OLD_PROCESS_STALLED', 'brand_0001', 'comment',
		 'legal_0001', 'legal_0002', 'legal_0003'
    ...
    is_deeply( $xlator->strings(), {   'encoding' => 'UTF-8',
				       'xml_version' => '1.0'},
...
sub lookup_msg {
    ...
    my $tag = {key => 'OLD_PROCESS_CRASH',
	       language => 'en_CA',
    };

    is( $xlator->lookup_msg( $$, $tag ), 'Old process crashed.',
	'lookup_msg() OK' );
