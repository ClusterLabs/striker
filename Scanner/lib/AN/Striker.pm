...
sub get_peer_node {
    ...
    foreach my $this_node ( sort { $a cmp $b }
                            @{ $conf->{clusters}{$cluster}{nodes} } ) {
    ...
    if ( not $peer ) {
        AN::Cluster::error(
            $conf,
            "I was asked to find the peer to: [$node], but failed. This is likely a program error.\n"
        );
    }
    ...
    if ( $conf->{cgi}{task} eq "withdraw" ) {
        ...
        if ( $conf->{cgi}{confirm} ) {
            if ( verify_vm_config($conf) ) {
                ...
                # We're golden
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; VM verified, creating now.\n" );
                provision_vm($conf);
            }
            else {
                # Something wasn't sane.
                AN::Cluster::record( $conf,
                     "$THIS_FILE " . __LINE__ . "; VM verification failed.\n" );
        ...
        if (    ( not $conf->{storage}{is}{lsi} )
            ...
            # No managers found
            my $say_title =
                AN::Common::get_string( $conf,
                                        {  key => "title_0016",
                                           variables =>
                                               { node => $conf->{cgi}{node}, } }
                                      );
            my $say_message =
                AN::Common::get_string(
                    $conf,
                    { key       => "message_0051",
                      variables => {
                           node_cluster_name => $conf->{cgi}{node_cluster_name},
                      } } );
            print AN::Common::template( $conf,
                                        "lsi-storage.html",
                                        "no-managers-found",
                                        {  title   => $say_title,
                                           message => $say_message,
...
sub restart_tomcat {
    ...
    if ( not $quiet ) {
        ...
        print AN::Common::template( $conf, "common.html",
                                    "restart-guacamole-header" );
    }
    ...
    while ( call_restart_tomcat_guacd( $conf, $quiet ) ) {
        ...
        if ( $tries > $max_tries ) {
            if ($quiet) {
                ...
                print AN::Common::template( $conf, "common.html",
                             "shell-call-output", { line => "message_0336", } );
            }
            last;
        }
        else {
            if ($quiet) {
                ...
                print AN::Common::template( $conf, "common.html",
                             "shell-call-output", { line => "message_0335", } );
            }
        }
        sleep 5;
    }
...
sub call_restart_tomcat_guacd {
    ...
    my $retry = 0;
    my $sc
        = "$conf->{path}{restart_tomcat} restart && $conf->{path}{restart_guacd} restart";
    AN::Cluster::record( $conf,
                         "$THIS_FILE " . __LINE__ . "; Calling: [$sc]\n" );
    my $fh = IO::Handle->new();
    open( $fh, "$sc 2>&1 |" )
        or die "$THIS_FILE " . __LINE__ . "; Failed to call: [$sc]\n";
    while (<$fh>) {
        chomp;
        my $line = $_;
        if ( ( $line =~ /Starting/i ) && ( $line =~ /Failed/i ) ) {
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; Failure detected: [$line]. Will retry.\n"
                               );
            $retry = 1;
        }
        if ($quiet) {
            AN::Cluster::record( $conf,
                               "$THIS_FILE " . __LINE__ . "; line: [$line]\n" );
        }
        else {
...
sub lsi_control_unmake_disk_as_hot_spare {
    ...
    my ( $error, $ssh_fh, $output )
        = AN::Cluster::remote_call(
        $conf,
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDHSP Rmv PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
                    . __LINE__
                    . "; error: [$error], ssh_fh: [$ssh_fh], output: [$output ("
                    . @{$output}
                    . " lines)]\n" );
...
sub lsi_control_clear_foreign_state {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
           "$THIS_FILE " . __LINE__ . "; lsi_control_clear_foreign_state()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 0,
           shell_call =>
               "$conf->{storage}{is}{lsi} CfgForeign Clear -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_make_disk_hot_spare {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
           "$THIS_FILE " . __LINE__ . "; lsi_control_make_disk_hot_spare()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 0,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDHSP Set PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_mark_disk_missing {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
             "$THIS_FILE " . __LINE__ . "; lsi_control_mark_disk_missing()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDMarkMissing PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
    ...
    foreach my $line ( @{$output} ) {
        ...
        if ( $line =~ /is marked missing/i ) {
            $success = 1;
             ...
             $line =~ /in a state that doesn't support the requested command/i )
        {
            $success = 2;
        }
    }
...
sub lsi_control_spin_disk_up {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
                  "$THIS_FILE " . __LINE__ . "; lsi_control_spin_disk_up()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDPrpRmv Undo PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_spin_disk_down {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
                "$THIS_FILE " . __LINE__ . "; lsi_control_spin_disk_down()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDPrpRmv PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_get_rebuild_progress {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; lsi_control_get_rebuild_progress(); disk_address: [$disk_address], adapter: [$adapter]\n"
                       );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDRbld ShowProg PhysDrv [$disk_address] a$adapter",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_put_disk_offline {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
              "$THIS_FILE " . __LINE__ . "; lsi_control_put_disk_offline()\n" );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], State: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{'state'}]\n"
                       );
    if (
         ( $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
           {$this_logical_disk}{'state'} eq "Degraded" )
         && ( $this_logical_disk != 9999 )
        ) {
        my $reason = AN::Common::get_string( $conf, { key => "lsi_0019" } );
        if ( $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
             {$this_logical_disk}{primary_raid_level} eq "6" ) {
    ...
    if ( not $conf->{cgi}{confirm} ) {
        ...
        $message .=
            ...
            {  key       => "lsi_0024",
               variables => {
                   confirm_url =>
                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=put_disk_offline&disk_address=$conf->{cgi}{disk_address}&adapter=$this_adapter&logical_disk=$this_logical_disk&confirm=true",
                   cancel_url =>
                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health",
               } } );
        print AN::Common::template( $conf,
                                    "lsi-storage.html",
                                    "lsi-complete-table-message",
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDOffline PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_put_disk_online {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
               "$THIS_FILE " . __LINE__ . "; lsi_control_put_disk_online()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDRbld Start PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_add_disk_to_array {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
             "$THIS_FILE " . __LINE__ . "; lsi_control_add_disk_to_array()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PdReplaceMissing PhysDrv [$conf->{cgi}{disk_address}] -array$conf->{cgi}{logical_disk} -row$conf->{cgi}{row} -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
    ...
    foreach my $line ( @{$output} ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE " . __LINE__ . "; line: [$line]\n" );
        if (    ( $line =~ /successfully added the disk/i )
             || ( $line =~ /missing pd at array/i ) ) {
            $success = 1;
        }
    }
...
sub lsi_control_get_missing_disks {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; lsi_control_get_missing_disks(); this_adapter: [$this_adapter] this_logical_disk: [$this_logical_disk]\n"
                       );
    ...
    my ( $error, $ssh_fh, $output )
        = AN::Cluster::remote_call(
                 ...
                 { node     => $node,
                   ...
                   'close'  => 1,
                   shell_call =>
                       "$conf->{storage}{is}{lsi} PdGetMissing a$this_adapter",
                 } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
    ...
    foreach my $line ( @{$output} ) {
        ...
        if ( $line =~ /\d+\s+\d+\s+(\d+)\s(\d+)/i ) {
            ...
            $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                {$this_logical_disk}{missing_row}{$this_row} = $minimum_size;
        }
    }
...
sub lsi_control_make_disk_good {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
                "$THIS_FILE " . __LINE__ . "; lsi_control_make_disk_good()\n" );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PDMakeGood PhysDrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub lsi_control_disk_id_led {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; lsi_control_disk_id_led(); action: [$action]\n"
                       );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; action: [$action], say_action: [$say_action]\n"
                       );
    ...
    my ( $error, $ssh_fh, $output )
        ...
        {  node     => $node,
           ...
           'close'  => 1,
           shell_call =>
               "$conf->{storage}{is}{lsi} PdLocate $action physdrv [$conf->{cgi}{disk_address}] -a$conf->{cgi}{adapter}",
        } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
        ...
        foreach my $this_adapter ( sort { $a cmp $b }
            ...
            foreach my $this_logical_disk ( sort { $a cmp $b }
                ...
                foreach my $this_enclosure_device_id (
                               sort { $a cmp $b }
                               keys %{
                                   $conf->{storage}{lsi}{adapter}{$this_adapter}
                                       {logical_disk}{$this_logical_disk}
                                       {enclosure_device_id} }
                    ) {
                    foreach my $this_slot_number (
                               sort { $a cmp $b }
                               keys %{
                                   $conf->{storage}{lsi}{adapter}{$this_adapter}
                                       {logical_disk}{$this_logical_disk}
                                       {enclosure_device_id}
                                       {$this_enclosure_device_id}{slot_number}
                                     }
                        ) {
...
#$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{raw_sector_count_in_hex}
            ...
            #print "</pre>\n";
            my $say_bbu
                = $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu_is}
                ? "Present"
                : "Not Installed";
            my $say_flash
                = $conf->{storage}{lsi}{adapter}{$this_adapter}{flash_is}
                ? "Present"
                : "Not Installed";
            my $say_restore_hotspare_on_insert
                = $conf->{storage}{lsi}{adapter}{$this_adapter}
                {restore_hotspare_on_insertion} ? "Yes" : "No";
            my $say_title =
                AN::Common::get_string( $conf,
                                        {  key => "lsi_0040",
                                           variables =>
            ...
            print AN::Common::template(
                  $conf,
                  "lsi-storage.html",
                  "lsi-adapter-state",
                  { title => $say_title,
                    model => $conf->{storage}{lsi}{adapter}{$this_adapter}
                        {product_name},
                    cache_size => $conf->{storage}{lsi}{adapter}{$this_adapter}
                        {cache_size},
                    bbu          => $say_bbu,
                    flash_module => $say_flash,
                    restore_hotspare_on_insert =>
                        $say_restore_hotspare_on_insert,
                  } );
            if ( $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu_is} ) {
                my $say_replace_bbu
                    = $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                    {replace_bbu} ? "Yes" : "No";
                my $say_learn_cycle
                    = $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                    {learn_cycle_active} ? "Yes" : "No";
                my $battery_state_class = "highlight_good";
                if ( $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                     {battery_state} ne "Optimal" ) {
                ...
                my $say_cycle_count
                    = "<span class=\"highlight_unavailable\">#!string!state_0016!#</a>";
                if ( $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                     {full_capacity} ) {
                    $say_current_capacity
                        = $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                        {full_capacity};
                }
                elsif ( $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                        {capacitance} ) {
                    $say_current_capacity
                        = "$conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{capacitance} %";
                }
                if ( $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                     {design_capacity} ) {
                    $say_design_capcity
            ...
            foreach my $this_logical_disk ( sort { $a cmp $b }
                ...
                my $say_bad_blocks
                    = $conf->{storage}{lsi}{adapter}{$this_adapter}
                    {logical_disk}{$this_logical_disk}{bad_blocks_exist}
                    ? "Yes"
                    : "No";
                my $say_size = $conf->{storage}{lsi}{adapter}{$this_adapter}
                    {logical_disk}{$this_logical_disk}{size};
                $say_size
                    =~ s/(\w)B/$1iB/;    # The use 'GB' when it should be 'GiB'.
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], State: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{'state'}]\n");
                if ( $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                     {$this_logical_disk}{'state'} eq "Degraded" ) {
                    lsi_control_get_missing_disks( $conf, $this_adapter,
                                                   $this_logical_disk );
                    $allow_offline            = 0;
                    ...
                    foreach my $this_row (
                         ...
                         keys %{
                             $conf->{storage}{lsi}{adapter}{$this_adapter}
                                 {logical_disk}{$this_logical_disk}{missing_row}
                               }
                        ) {
                        my $say_minimum_size
                            = AN::Cluster::bytes_to_hr( $conf,
                             $conf->{storage}{lsi}{adapter}{$this_adapter}
                                 {logical_disk}{$this_logical_disk}{missing_row}
                                 {$this_row} );
                        $say_missing .=
                ...
                if ( $this_logical_disk == 9999 ) {
                    ...
                    # disk state.
                    print AN::Common::template( $conf, "lsi-storage.html",
                                                "lsi-unassigned-disks" );
                }
                else {
                    # Real logical disk
                    next
                        if not $conf->{storage}{lsi}{adapter}{$this_adapter}
                        {logical_disk}{$this_logical_disk}{'state'};
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], State: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{'state'}]\n");
                    my $title =
                        AN::Common::get_string(
                         $conf,
                         { key       => "title_0021",
                           variables => { logical_disk => $this_logical_disk, }
                         } );
                    print AN::Common::template(
                        $conf,
                        "lsi-storage.html",
                        "lsi-logical-disk-state",
                        {  title => $title,
                           logical_disk_state =>
                               $conf->{storage}{lsi}{adapter}{$this_adapter}
                               {logical_disk}{$this_logical_disk}{'state'},
                           logical_disk_state_class =>
                               $logical_disk_state_class,
                           missing          => $say_missing,
                           ...
                           primary_raid_level =>
                               ...
                               {primary_raid_level},
                           raid_url =>
                               "https://alteeve.ca/w/TLUG_Talk:_Storage_Technologies_and_Theory#Level_$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{primary_raid_level}",
                           size => $say_size,
                           number_of_drives =>
                               $conf->{storage}{lsi}{adapter}{$this_adapter}
                               {logical_disk}{$this_logical_disk}
                           ...
                           target_id =>
                               $conf->{storage}{lsi}{adapter}{$this_adapter}
                               {logical_disk}{$this_logical_disk}{target_id},
        ...
        #print " | |- Enclusure Device ID: [$this_enclosure_device_id]<br />\n";
                    foreach my $this_slot_number (
                        ...
                        my $raw_size_sectors
                            = hex( $conf->{storage}{lsi}{adapter}{$this_adapter}
                                 {logical_disk}{$this_logical_disk}
                                 {enclosure_device_id}
                                 {$this_enclosure_device_id}{slot_number}
                                 {$this_slot_number}{raw_sector_count_in_hex} );
                        my $raw_size_bytes
                            = (   $raw_size_sectors
                                * $conf->{storage}{lsi}{adapter}{$this_adapter}
                                {logical_disk}{$this_logical_disk}
                                {enclosure_device_id}
                                {$this_enclosure_device_id}{slot_number}
                                {$this_slot_number}{sector_size} );
                        my $say_raw_size = AN::Cluster::bytes_to_hr( $conf,
                                                              $raw_size_bytes );
                        my $disk_temp_class = "highlight_good";
                        if ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                             {logical_disk}{$this_logical_disk}
                             {enclosure_device_id}{$this_enclosure_device_id}
                             {slot_number}{$this_slot_number}{drive_temp_c}
                             > 54 ) {
                            $disk_temp_class = "highlight_dangerous";
                        }
                        elsif ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                                {logical_disk}{$this_logical_disk}
                                {enclosure_device_id}{$this_enclosure_device_id}
                                {slot_number}{$this_slot_number}{drive_temp_c}
                                > 45 ) {
                        ...
                        my $say_drive_temp_f
                            ...
                            {logical_disk}{$this_logical_disk}
                            {enclosure_device_id}{$this_enclosure_device_id}
                            {slot_number}{$this_slot_number}{drive_temp_f}
                            ? $conf->{storage}{lsi}{adapter}{$this_adapter}
                            {logical_disk}{$this_logical_disk}
                            {enclosure_device_id}{$this_enclosure_device_id}
                            {slot_number}{$this_slot_number}{drive_temp_f}
                            : "--";
                        my $say_temperature
                            = "<span class=\"$disk_temp_class\">$say_drive_temp_c &deg;C ($say_drive_temp_f &deg;F)</span>";

                        my $say_location_title = AN::Common::get_string( $conf,
                                                        { key => "row_0066" } );
                        my $say_location_body
                            = "$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{span}, $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{arm}, $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{device_id}";
                        if ( $this_logical_disk == 9999 ) {
                            $say_location_title = AN::Common::get_string( $conf,
                                                        { key => "row_0067" } );
                            $say_location_body
                                = "$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{device_id}";
                        }

                        my $say_offline_disabled_button
                            = AN::Common::template(
                                             $conf,
                                             "common.html",
                                             "disabled-button",
                        ...
                        if ($allow_offline) {
                            $offline_button
                                ...
                                $conf,
                                "common.html",
                                "enabled-button-no-class",
                                {  button_link =>
                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=put_disk_offline&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter&logical_disk=$this_logical_disk",
                                   button_text => "#!string!button_0006!#",
                                   id =>
                                       "put_disk_offline_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                },
                                "", 1 );
                            if ( $this_logical_disk == 9999 ) {
                                $offline_button
                                    ...
                                    "enabled-button-no-class",
                                    {  button_link =>
                                           "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=spin_disk_down&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter&logical_disk=$this_logical_disk",
                                       button_text => "#!string!button_0007!#",
                                       id =>
                                           "spin_disk_down_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                    },
                                    "", 1 );
                            }
                        }
                        ...
                        if ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                             {logical_disk}{$this_logical_disk}
                             {enclosure_device_id}{$this_enclosure_device_id}
                             {slot_number}{$this_slot_number}{firmware_state} eq
                             "Unconfigured(bad)" ) {
                            $disk_state_class = "highlight_bad";
                            $say_disk_action
                                = AN::Common::template( $conf, "common.html",
                                                        "new_line", "", "", 1 );
                            $say_disk_action .=
                                AN::Common::template(
                                $conf,
                                ...
                                {  button_class => $disk_state_class,
                                   button_link =>
                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=make_disk_good&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter",
                                   button_text => "#!string!button_0008!#",
                                   id =>
                                       "make_disk_good_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                },
                                "", 1 );
                        }
                        elsif ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                            ...
                            my $say_put_disk_online_button
                                ...
                                {  button_class => $disk_state_class,
                                   button_link =>
                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=put_disk_online&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter",
                                   button_text => "#!string!button_0009!#",
                                   id =>
                                       "put_disk_online__${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                },
                                "", 1 );
                            my $say_spin_disk_down_button
                                = AN::Common::template(
                                $conf,
                                "common.html",
                                "enabled-button_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                {  button_class => $disk_state_class,
                                   button_link =>
                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=spin_disk_down&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter",
                                   button_text => "#!string!button_0007!#",
                                   id          => "spin_disk_down",
                                },
                                "", 1 );
                            $say_disk_action
                                = " - $say_put_disk_online_button - $say_spin_disk_down_button";
                        }
                        elsif ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                             {logical_disk}{$this_logical_disk}
                             {enclosure_device_id}{$this_enclosure_device_id}
                             {slot_number}{$this_slot_number}{firmware_state} eq
                             "Rebuild" ) {
                            my ( $rebuild_percent, $time_to_complete )
                                = lsi_control_get_rebuild_progress( $conf,
                                  "$this_enclosure_device_id:$this_slot_number",
                                  $this_adapter );
                            ...
                            foreach my $this_logical_disk (
                                           sort { $a cmp $b }
                                           keys %{
                                               $conf->{storage}{lsi}{adapter}
                                                   {$this_adapter}{logical_disk}
                                                 }
                                ) {
                                next if $this_logical_disk eq "";
                                if ( $conf->{storage}{lsi}{adapter}
                                     {$this_adapter}{logical_disk}
                                     {$this_logical_disk}{'state'} eq
                                     "Degraded" ) {
                                    foreach my $this_row (
                                           sort { $a cmp $b }
                                           keys %{
                                        ...
                                        if ( $raw_size_bytes
                                             >= $conf->{storage}{lsi}{adapter}
                                             {$this_adapter}{logical_disk}
                                             {$this_logical_disk}{missing_row}
                                             {$this_row} ) {
                                            $say_disk_action
                                                = AN::Common::template( $conf,
                                                  "common.html", "new_line", "",
                                                  "", 1 )
                                            ...
                                            $say_disk_action .=
                                                ...
                                                {  button_class =>
                                                       "bold_button",
                                                   button_link =>
                                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=add_disk_to_array&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter&row=$this_row&logical_disk=$this_logical_disk",
                                                   button_text => $say_button,
                                                   id =>
                                                       "add_disk_to_array_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                                },
                                                "", 1 );
                                            $say_disk_action .=
                                                AN::Common::template( $conf,
                                    ...
                                    $say_disk_action .=
                                        ...
                                        "enabled-button-no-class",
                                        {  button_link =>
                                               "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=make_disk_hot_spare&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter",
                                           button_text =>
                                               "#!string!button_0011!#",
                                           id =>
                                               "make_disk_hot_spare_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                        },
                                        "", 1 );
                                    $say_disk_action .=
                                        AN::Common::template( $conf,
...
#$offline_button   = "<a href=\"?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=make_disk_hot_spare&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter\">".AN::Common::get_string($conf, {key => "button_0011"})."</a>";
                            ...
                            $say_disk_action .=
                                ...
                                "enabled-button-no-class",
                                {  button_link =>
                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=spin_disk_up&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter",
                                   button_text => "#!string!button_0012!#",
                                   id =>
                                       "spin_disk_up_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                },
                                "", 1 );
                            $say_temperature
                                = "<span class=\"highlight_unavailable\">"
                                . AN::Common::get_string( $conf,
                                                     { key => "message_0055" } )
                                . "</a>";
                            $offline_button = AN::Common::get_string( $conf,
                                                    { key => "message_0054" } );
                        }
                        elsif ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                             {logical_disk}{$this_logical_disk}
                            ...
                            $say_disk_action
                                = AN::Common::template( $conf, "common.html",
                                                        "new_line", "", "", 1 );
                            $say_disk_action .= AN::Common::get_string( $conf,
                                                    { key => "message_0056" } );
                            $say_temperature
                                = "<span class=\"highlight_unavailable\">"
                                . AN::Common::get_string( $conf,
                            ...
                            $offline_button
                                ...
                                "enabled-button-no-class",
                                {  button_link =>
                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=unmake_disk_as_hot_spare&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter&logical_disk=$this_logical_disk",
                                   button_text => "#!string!button_0013!#",
                                   id =>
                                       "unmake_disk_as_hot_spare_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                },
                                "", 1 );
                        }
                        ...
                        my $disk_icon
                            = "$conf->{url}{skins}/$conf->{sys}{skin}/images/hard-drive_128x128.png";
                        my $id_led_url
                            = "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=start_id_disk&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter";
                        my $say_identify = AN::Common::get_string( $conf,
                                                     { key => "button_0014" } );
...
#print "adapter: [$this_adapter], Disk: [$this_enclosure_device_id:$this_slot_number], Locator ID Status: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{id_led_lit}]\n";
                        if ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                             {logical_disk}{$this_logical_disk}
                             {enclosure_device_id}{$this_enclosure_device_id}
                             {slot_number}{$this_slot_number}{id_led_lit} ) {
                            $disk_icon
                                = "$conf->{url}{skins}/$conf->{sys}{skin}/images/hard-drive-with-led_128x128.png";
                            $id_led_url
                                = "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=stop_id_disk&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter";
                            $say_identify = AN::Common::get_string( $conf,
                                                     { key => "button_0015" } );
                        }
                        ...
                        if ( $conf->{storage}{lsi}{adapter}{$this_adapter}
                             {logical_disk}{$this_logical_disk}
                             {enclosure_device_id}{$this_enclosure_device_id}
                             {slot_number}{$this_slot_number}{foreign_state} eq
                             "Foreign" ) {
                            $foreign_state_class
                                = "highlight_warning_bold_fixed_width_left";
                            ...
                            $say_disk_action .=
                                ...
                                "enabled-button-no-class",
                                {  button_link =>
                                       "?cluster=$conf->{cgi}{cluster}&node=$conf->{cgi}{node}&node_cluster_name=$conf->{cgi}{node_cluster_name}&task=display_health&do=clear_foreign_state&disk_address=$this_enclosure_device_id:$this_slot_number&adapter=$this_adapter",
                                   button_text => "#!string!button_0038!#",
                                   id =>
                                       "clear_foreign_state_${this_adapter}_${this_enclosure_device_id}_${this_slot_number}",
                                },
                                "", 1 );
                        }
                        ...
                        $conf->{storage}{lsi}{adapter}{$this_adapter}
                            {logical_disk}{$this_logical_disk}
                            {enclosure_device_id}{$this_enclosure_device_id}
                            {slot_number}{$this_slot_number}{sas_address_1}
                            = "--"
                            if not $conf->{storage}{lsi}{adapter}{$this_adapter}
                            {logical_disk}{$this_logical_disk}
                            {enclosure_device_id}{$this_enclosure_device_id}
                        ...
                        print AN::Common::template(
                              $conf,
                              "lsi-storage.html",
                              "lsi-physical-disk-state",
                              { title      => $title,
                                id_led_url => $id_led_url,
                                disk_icon  => $disk_icon,
                                model      => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{inquiry_data},
                                wwn => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{wwn},
                                disk_state => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{firmware_state},
                                ...
                                device_type => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{media_type},
                                media_error_count =>
                                    $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{media_error_count},
                                other_error_count =>
                                    $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{other_error_count},
                                ...
                                pd_type        => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{pd_type},
                                device_speed => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{device_speed},
                                link_speed => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{link_speed},
                                sas_address_0 => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{sas_address_0},
                                sas_address_1 => $conf->{storage}{lsi}{adapter}
                                    {$this_adapter}{logical_disk}
                                    {$this_logical_disk}{enclosure_device_id}
                                    {$this_enclosure_device_id}{slot_number}
                                    {$this_slot_number}{sas_address_1},
...
sub get_storage_data {
    ...
    my ( $error, $ssh_fh, $output ) = AN::Cluster::remote_call(
        $conf,
        {  node     => $node,
           port     => $conf->{node}{$node}{port},
           user     => "root",
...
sub get_storage_data_lsi {
    ...
    my $shell_call
        = "echo '==] Start adapter_info'; $megacli64_path AdpAllInfo aAll; ";
    $shell_call
        .= "echo '==] Start bbu_info'; $megacli64_path AdpBbuCmd aAll; ";
    $shell_call
        .= "echo '==] Start logical_disk_info'; $megacli64_path LDInfo Lall aAll; ";
    $shell_call
        .= "echo '==] Start physical_disk_info'; $megacli64_path PDList aAll; ";
    $shell_call
        .= "echo '==] Start pd_id_led_state'; $conf->{path}{grep} \"PD Locate\" /root/MegaSAS.log;";
    AN::Cluster::record( $conf,
                   "$THIS_FILE " . __LINE__ . "; shell_call: [$shell_call]\n" );
    my ( $error, $ssh_fh, $output )
        = AN::Cluster::remote_call(
                                $conf,
                                { node     => $node,
...
#print "LSI Adapter number: [$this_adapter], BBU State: [$conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{battery_state}]\n";
                ...
                $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}
                    {learn_cycle_active} = $learn_cycle_active eq "Yes" ? 1 : 0;
...
#print "LSI Adapter number: [$this_adapter], BBU Learn Cycle Active: [$conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{learn_cycle_active} ($learn_cycle_active)]\n";
                  ...
                  =~ /Pack is about to fail & should be replaced\s*:\s*(.*)/ ) {
                my $replace_bbu = $1;
                $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{replace_bbu}
                    = $replace_bbu eq "Yes" ? 1 : 0;
...
#print "LSI Adapter number: [$this_adapter], BBU Remaining Capacity: [$conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{remaining_capacity}]\n";
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; LSI Adapter number: [$this_adapter], BBU Full Capacity: [$conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{full_capacity}]\n"
                                   );
            }
...
#print "LSI Adapter number: [$this_adapter], BBU Manufactore Name: [$conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{manufacture_name}]\n";
                ...
                $conf->{storage}{lsi}{adapter}{$this_adapter}{bbu}{pack_energy}
                    = $1;
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], State: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{'state'}]\n");
                ...
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{current_cache_policy} = $1;
...
#print "LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Current Cache Policy: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{current_cache_policy}]\n";
                ...
                my $bad_blocks = $1;
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{bad_blocks_exist}
                    = $bad_blocks eq "Yes" ? 1 : 0;
...
#print "LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Bad Blocks Exist: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{bad_blocks_exist} ($bad_blocks)]\n";
                ...
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{number_of_drives} = $1;
...
#print "LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Number of drives: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{number_of_drives}]\n";
                ...
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{encryption_type} = $1;
...
#print "LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Encryption Type: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{encryption_type}]\n";
                ...
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{disk_cache_policy} = $1;
...
#print "LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Disk Cache Policy: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{disk_cache_policy}]\n";
                ...
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{sector_size} = $1;
...
#print "LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Sector size: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{sector_size}]\n";
                ...
                =~ /RAID Level\s*:\s*Primary-(\d+), Secondary-(\d+), RAID Level Qualifier-(\d+)/
                ) {
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{primary_raid_level} = $1;
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{secondary_raid_level} = $2;
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{raid_qualifier} = $3;
...
#print "LSI Adapter number: [$this_adapter], Physical Disk, Slot Number: [$this_slot_number].\n";
            ...
            if ( $line
                =~ /Drive's position: DiskGroup: (\d+), Span: (\d+), Arm: (\d+)/
                ) {
                $this_logical_disk = $1;
                $this_span         = $2;
                $this_arm          = $3;
...
#print "LSI Adapter number: [$this_adapter], Disk Group: [$this_logical_disk], Enclosure Device ID: [$this_enclosure_device_id], Slot Number: [$this_slot_number].\n";
            ...
            if (    ( $line =~ /Enclosure position: N\/A/ )
                 && ( $this_logical_disk eq "" ) ) {
...
#print "LSI Adapter number: [$this_adapter], Disk Group: [$this_logical_disk], Enclosure Device ID: [$this_enclosure_device_id], Slot Number: [$this_slot_number].\n";
            ...
            next
                if (    ( $this_enclosure_device_id eq "" )
                     or ( $this_slot_number eq "" )
                     or ( $this_logical_disk eq "" ) );

            if ( not exists $conf->{storage}{lsi}{adapter}{$this_adapter}
                 {logical_disk}{$this_logical_disk}{enclosure_device_id}
                 {$this_enclosure_device_id}{slot_number}{$this_slot_number}
                 {span} ) {
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{enclosure_device_id}
                    {$this_enclosure_device_id}{slot_number}{$this_slot_number}
...
#print "LSI Adapter number: [$this_adapter], Disk Group: [$this_logical_disk], Enclosure Device ID: [$this_enclosure_device_id], Slot Number: [$this_slot_number].\n";
            ...
            if (not exists $conf->{storage}{lsi}{adapter}{$this_adapter}
                {logical_disk}{$this_logical_disk}{enclosure_device_id}
                {$this_enclosure_device_id}{slot_number}{$this_slot_number}{arm}
                ) {
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{enclosure_device_id}
                    {$this_enclosure_device_id}{slot_number}{$this_slot_number}
...
#print "LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Enclosure Device ID: [$this_enclosure_device_id], Slot Number: [$this_slot_number], Raw Sector Count in Hex: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{raw_sector_count_in_hex}]\n";
                ...
                if ( not $sector_size ) {
                    $sector_size
                        = $conf->{storage}{lsi}{adapter}{$this_adapter}
                        {logical_disk}{$this_logical_disk}{sector_size}
                        ? $conf->{storage}{lsi}{adapter}{$this_adapter}
                        {logical_disk}{$this_logical_disk}{sector_size}
                        : 512;
                }
                $conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}
                    {$this_logical_disk}{enclosure_device_id}
                    {$this_enclosure_device_id}{slot_number}{$this_slot_number}
                    {sector_size} = $sector_size ? $sector_size : 512;
...
#print __LINE__.";        - LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Enclosure Device ID: [$this_enclosure_device_id], Slot Number: [$this_slot_number], id_led_lit: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{id_led_lit}]\n";
            ...
            if ( $line
                =~ /adapter: (\d+): device at enclid-(\d+) slotid-(\d+) -- pd locate (.*?) command was successfully sent to firmware/
                ) {
                $this_adapter             = $1;
...
#print __LINE__.";   - Adapter: [$this_logical_disk], Logical Disk: [$this_logical_disk], Enclosure Device ID: [$this_enclosure_device_id], Slot Number: [$this_slot_number]\n";
                        if ( exists $conf->{storage}{lsi}{adapter}
                             {$this_adapter}{logical_disk}{$this_logical_disk}
                             {enclosure_device_id}{$this_enclosure_device_id}
                             {slot_number}{$this_slot_number} ) {
                            $conf->{storage}{lsi}{adapter}{$this_adapter}
                                {logical_disk}{$this_logical_disk}
                                {enclosure_device_id}
                                {$this_enclosure_device_id}{slot_number}
                                {$this_slot_number}{id_led_lit} = $set_state;
...
#print __LINE__.";     - Set State: LSI Adapter number: [$this_adapter], Logical Disk: [$this_logical_disk], Enclosure Device ID: [$this_enclosure_device_id], Slot Number: [$this_slot_number], id_led_lit: [$conf->{storage}{lsi}{adapter}{$this_adapter}{logical_disk}{$this_logical_disk}{enclosure_device_id}{$this_enclosure_device_id}{slot_number}{$this_slot_number}{id_led_lit}]\n";
            ...
            die "$THIS_FILE " . __LINE__
                . "; unknown section!, line: [$line]\n";
        }
    }
        ...
        foreach my $this_logical_disk ( sort { $a cmp $b }
            ...
            foreach my $this_enclosure_device_id (
                ...
                foreach my $this_slot_number (
                    ...
                    if ( exists $conf->{storage}{lsi}{adapter}{$this_adapter}
                         {logical_disk}{$this_logical_disk}{enclosure_device_id}
                         {$this_enclosure_device_id}{slot_number}
                         {$this_slot_number}{id_led_lit} ) {
...
sub change_vm {
    ...
    my $available_ram
        = (   $conf->{resources}{total_ram}
            - $conf->{'system'}{unusable_ram}
            - $conf->{resources}{allocated_ram} )
        + $current_ram;
    $current_ram /= 1024;
    my $requested_ram =
        AN::Cluster::hr_to_bytes( $conf,
                                  $conf->{cgi}{ram},
                                  $conf->{cgi}{ram_suffix}, 1 );
    $requested_ram /= 1024;
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; - current CPUs:   [$current_cpus]\n");
    ...
    # Open the table.
    my $title = AN::Common::get_string( $conf,
                                        {  key       => "title_0023",
                                           variables => { server => $say_vm, } }
                                      );
    print AN::Common::template( $conf, "server.html",
                                "update-server-config-header",
                                { title => $title, } );

    # Make sure something changed.
    if (    ( $current_ram ne $requested_ram )
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; requested ram:    [$requested_ram]\n" );
        AN::Cluster::record( $conf,
                "$THIS_FILE " . __LINE__ . ";       max ram:    [$max_ram]\n" );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__.";       max cpus:   [$max_cpus]\n");
        if ( $requested_ram > $max_ram ) {
            ...
            # Not enough RAM
            my $say_requested_ram
                = AN::Cluster::bytes_to_hr( $conf, ( $requested_ram * 1024 ) );
            my $say_max_ram
                = AN::Cluster::bytes_to_hr( $conf, ( $max_ram * 1024 ) );
            my $message =
                AN::Common::get_string( $conf,
                                        {  key       => "message_0059",
                                           variables => {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Changed line:    [$line]\n");
                ...
                if ( $line =~ /^(.*?)<memory unit='.*?'>\d+<\/memory>/ ) {
                    my $prefix = $1;
                    $line
                        = "${prefix}<memory unit='KiB'>$requested_ram<\/memory>";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Changed line:    [$line]\n");
                ...
                if ( $line =~ /^(.*?)<currentMemory>\d+<\/currentMemory>/ ) {
                    my $prefix = $1;
                    $line
                        = "${prefix}<currentMemory>$requested_ram<\/currentMemory>";

#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Changed line:    [$line]\n");
                }
                if ( $line
                     =~ /^(.*?)<currentMemory unit='.*?'>\d+<\/currentMemory>/ )
                {
                    my $prefix = $1;
                    $line
                        = "${prefix}<currentMemory unit='KiB'>$requested_ram<\/currentMemory>";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Changed line:    [$line]\n");
                ...
                if ( $line =~ /^(.*?)<vcpu placement='(.*?)'>(\d+)<\/vcpu>/ ) {
                    ...
                    my $placement = $2;
                    $line
                        = "${prefix}<vcpu placement='$placement'>$requested_cpus<\/vcpu>";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], exiting the OS block.\n");
                        ...
                        $new_definition
                            .= "    <boot dev='$requested_boot_device'/>\n";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; new definition: [$new_definition]\n");
            ...
            ( $error, $ssh_fh, $output )
                ...
                {  node     => $node,
                   ...
                   'close'  => 1,
                   shell_call =>
                       "echo \"$new_definition\" > $definition_file && chmod 644 $definition_file",
                } );
            AN::Cluster::record( $conf,
                      "$THIS_FILE "
                    . __LINE__
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm::${vm}::details::ram: [$conf->{vm}{$vm}{details}{ram}]\n");
            $conf->{resources}{allocated_ram}
                = $other_allocated_ram + ( $requested_ram * 1024 );
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; host: [$conf->{vm}{$vm}{current_host}]<br />\n"
                               );
            if ( $conf->{vm}{$vm}{current_host} ) {
                print AN::Common::template( $conf, "server.html",
                                           "server-poweroff-required-message" );
            }
            print AN::Common::template(
                $conf,
                "server.html",
                "update-server-config-footer",
                {  url =>
                       "?cluster=$conf->{cgi}{cluster}&vm=$conf->{cgi}{vm}&task=manage_vm",
                } );
    ...
    if ($vm_is_running) {
        ...
        my ( $error, $ssh_fh, $output )
            ...
            {  node     => $node,
               ...
               'close'  => 1,
               shell_call =>
                   "virsh change-media $say_vm $insert_drive --insert '/shared/files/$insert_media'; echo virsh:\$?",
            } );
        AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], line: [$line]\n");
        ...
        if ( $virsh_exit_code eq "1" ) {
            ...
            print AN::Common::template( $conf, "server.html",
                                        "insert-media-failed-already-mounted" );
            ...
            print AN::Common::template( $conf,
                                        "server.html",
                                        "insert-media-failed-bad-exit-code",
                                        { error => $say_error, } );
        }
    }
    else {
        ...
        #AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; line: [$line]\n");
            if ( $line =~ /dev='(.*?)'/ ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; Found the device: [$this_device].\n"
                                   );
                if ( $this_device eq $insert_drive ) {
                    $new_definition
                        .= "      <source file='/shared/files/$insert_media'/>\n";
                }
            }
            $new_definition .= "$line\n";
        }
    ...
    if ($vm_is_running) {
        ...
        my ( $error, $ssh_fh, $output )
            ...
            {  node     => $node,
               ...
               'close'  => 1,
               shell_call =>
                   "virsh change-media $say_vm $conf->{cgi}{device} --eject; echo virsh:\$?",
            } );
        AN::Cluster::record(
                $conf,
                "$THIS_FILE "
        ...
        foreach my $line ( @{$output} ) {
            ...
            next if not $line;
            AN::Cluster::record( $conf,
                "$THIS_FILE " . __LINE__ . "; node: [$node], line: [$line]\n" );
            if ( $line =~ /virsh:(\d+)/ ) {
                $virsh_exit_code = $1;
            }
            else {
        ...
        if ( $virsh_exit_code eq "1" ) {
            ...
            print AN::Common::template( $conf, "server.html",
                                        "eject-media-failed-already-ejected" );
            ...
            print AN::Common::template( $conf,
                                        "server.html",
                                        "eject-media-failed-bad-exit-code",
                                        { error => $say_error, } );
        }
    }
    else {
        ...
        #AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; line: [$line]\n");
            if ( ( $line =~ /type='file'/ ) && ( $line =~ /device='cdrom'/ ) ) {
                AN::Cluster::record( $conf,
                         "$THIS_FILE " . __LINE__ . "; Found a cdrom disk.\n" );
                $in_cdrom = 1;
            }
            if ($in_cdrom) {
                if ( $line =~ /file='(.*?)'\/>/ ) {
                ...
                if ( $line =~ /<\/disk>/ ) {
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                            . "; Checking if: [$this_device] is the device: [$conf->{cgi}{device}] I want to eject...\n"
                    );
                    if ( $this_device eq $conf->{cgi}{device} ) {

                        # This is the device I want to unmount.
                        AN::Cluster::record( $conf,
                                      "$THIS_FILE " . __LINE__ . "; It is!\n" );
                        $new_definition
                            =~ s/<disk(.*?)device='cdrom'(.*?)<source file='$this_media'\/>\s+(.*?)<\/disk>/<disk${1}device='cdrom'${2}${3}<\/disk>/s;
                    }
                    else {
                        AN::Cluster::record( $conf,
                                  "$THIS_FILE " . __LINE__ . "; It is not.\n" );
                    }
                    $in_cdrom    = 0;
                    $this_device = "";
                    $this_media  = "";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node::${node1}::daemon::gfs2::exit_code: [$conf->{node}{$node1}{daemon}{gfs2}{exit_code}], node::${node2}::daemon::gfs2::exit_code: [$conf->{node}{$node2}{daemon}{gfs2}{exit_code}]\n");
    if (    ( $conf->{node}{$node1}{daemon}{gfs2}{exit_code} )
         && ( $conf->{node}{$node2}{daemon}{gfs2}{exit_code} ) ) {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; vm::${vm}::current_host: [$conf->{vm}{$vm}{current_host}]\n"
                       );
    if ( $conf->{vm}{$vm}{current_host} ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; vm: [$vm] is running on: [$node], will read: [$conf->{vm}{$vm}{definition_file}].\n"
                           );
    }
    else {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; vm: [$vm] is not running, will read: [$conf->{vm}{$vm}{definition_file}] via: [$node].\n"
                           );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], OS boot line: [$line].\n");
                ...
                $boot_select
                    .= "<option value=\"$device\" $selected>$say_device</option>";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], Found a CDROM drive, adding it as a boot option.\n");
    ...
    $boot_select .= "</select>";
    AN::Cluster::record( $conf,
                "$THIS_FILE " . __LINE__ . "; boot select: [$boot_select].\n" );

    # If I need to change the number of CPUs or the amount of RAM, do so
    # now.
    AN::Cluster::record( $conf,
        "$THIS_FILE " . __LINE__ . "; cgi::change: [$conf->{cgi}{change}].\n" );
    if ( $conf->{cgi}{change} ) {
        change_vm( $conf, $node );
    }
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; do_insert: [$do_insert], insert_drive: [$insert_drive], insert_media: [$insert_media]\n"
                       );
    if ($do_insert) {
        vm_insert_media( $conf, $node, $insert_media, $insert_drive,
                         $vm_is_running );
    }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; total_space: [$conf->{partition}{shared}{total_space}], used_space: [$conf->{partition}{shared}{used_space} / $conf->{partition}{shared}{used_percent}%], free_space: [$conf->{partition}{shared}{free_space}]\n");
            ...
            =~ /^(\S)(\S+)\s+\d+\s+(\S+)\s+(\S+)\s+(\d+)\s+(\S+)\s+(\d+)\s+(\S+)\s+(.*)$/
            ) {
            my $type   = $1;
            my $mode   = $2;
            my $user   = $3;
            ...
            $conf->{files}{shared}{$file}{'time'}
                = $time;    # might be a year, look for '\d+:\d+'.
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], exiting a CD-ROM child element on: [$line].\n");
            $conf->{vm}{$vm}{cdrom}{$this_device}{media}
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; max_ram: [$max_ram (".AN::Cluster::bytes_to_hr($conf, $max_ram).")], max_cpu_count: [$max_cpu_count]\n");
    ...
    foreach my $device ( sort { $a cmp $b } keys %{ $conf->{vm}{$vm}{cdrom} } )
    {
        ...
        if ( $conf->{vm}{$vm}{cdrom}{$device}{media} ) {
            ...
            $conf->{vm}{$vm}{cdrom}{$device}{say_select}
                = "<select name=\"$key\" disabled>\n";
            $conf->{vm}{$vm}{cdrom}{$device}{say_in_drive}
                = "<span class=\"fixed_width\">$conf->{vm}{$vm}{cdrom}{$device}{media}</span>\n";
            $conf->{vm}{$vm}{cdrom}{$device}{say_eject}
                = AN::Common::template(
                $conf,
                "common.html",
                "enabled-button",
                {  button_class => "bold_button",
                   button_link =>
                       "?cluster=$conf->{cgi}{cluster}&vm=$conf->{cgi}{vm}&task=manage_vm&do=eject&device=$device",
                   button_text => "#!string!button_0017!#",
                   id          => "eject_$device",
                },
                "", 1 );
            my $say_insert_disabled_button
                = AN::Common::template(
                                   $conf,
                                   "common.html",
                                   "disabled-button",
                                   ...
                                   "",
                                   1 );
            $conf->{vm}{$vm}{cdrom}{$device}{say_insert}
                = "$say_insert_disabled_button\n";
        }
        else {
            ...
            $conf->{vm}{$vm}{cdrom}{$device}{say_select}
                = "<select name=\"$key\">\n";
            $conf->{vm}{$vm}{cdrom}{$device}{say_in_drive}
                = "<span class=\"highlight_unavailable\">(#!string!state_0007!#)</span>\n";
            my $say_eject_disabled_button
                = AN::Common::template(
                                   $conf,
                                   "common.html",
                                   "disabled-button",
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; file: [$file], cgi::${key}: [$conf->{cgi}{$key}]\n");
            if (    ( defined $conf->{cgi}{$key} )
                 && ( $file eq $conf->{cgi}{$key} ) ) {
                $conf->{vm}{$vm}{cdrom}{$device}{say_select}
                    .= "<option name=\"$file\" selected>$file</option>\n";
            }
            else {
                $conf->{vm}{$vm}{cdrom}{$device}{say_select}
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Media in: [$device] -> [$conf->{vm}{$vm}{cdrom}{$device}{media}]. [Select: $conf->{vm}{$vm}{cdrom}{$device}{say_select}]\n");
    ...
    my $select_cpu_cores =
        AN::Cluster::build_select( $conf, "cpu_cores", 0, 0, 60,
                                   $conf->{cgi}{cpu_cores}, $cpu_cores );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; select_cpu_cores: [$select_cpu_cores]\n");
    ...
    my $current_ram = $conf->{vm}{$vm}{details}{ram};
    AN::Cluster::record( $conf,
                 "$THIS_FILE " . __LINE__ . "; current_ram: [$current_ram]\n" );

    my $diff = $conf->{resources}{total_ram} % ( 1024**3 );
    my $available_ram
        = (   $conf->{resources}{total_ram}
            - $diff - $conf->{'system'}{unusable_ram}
            - $conf->{resources}{allocated_ram} )
        + $current_ram;
    my $max_ram = $available_ram;
    AN::Cluster::record( $conf,
             "$THIS_FILE " . __LINE__ . "; available_ram: [$available_ram]\n" );
    ...
    my ( $current_ram_value, $current_ram_suffix )
        = ( split / /, $say_current_ram );
    $conf->{cgi}{ram} = $current_ram_value if not $conf->{cgi}{ram};
    $conf->{cgi}{ram_suffix} = $current_ram_suffix
        if not $conf->{cgi}{ram_suffix};
    my $select_ram_suffix =
        AN::Cluster::build_select( $conf, "ram_suffix", 0, 0, 60,
                                   $conf->{cgi}{ram_suffix},
                                   [ "MiB", "GiB" ] );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; (<span class=\"subtle_text\">Maximum $say_max_ram</span>) <input type=\"input\" name=\"ram\" value=\"$conf->{cgi}{ram}\" style=\"width: 100px;\"> $select_ram_suffix\n");
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; path::guacamole_config: [$conf->{path}{guacamole_config}]\n"
                       );
    if ( -e $conf->{path}{guacamole_config} ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node: [$node], Guacamole installed, getting VNC info.\n"
                           );
        ( $node, my $type, my $listen, my $port )
            = get_current_vm_vnc_info( $conf, $vm, $node );
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node: [$node], type: [$type], listen: [$listen], port: [$port]\n"
                           );
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; vm::${vm}::graphics::type: [$conf->{vm}{$vm}{graphics}{type}],  vm::${vm}::graphics::listen: [$conf->{vm}{$vm}{graphics}{'listen'}]\n"
                           );
        if (    ( $conf->{vm}{$vm}{graphics}{type} ne "vnc" )
             || ( $conf->{vm}{$vm}{graphics}{'listen'} ne "0.0.0.0" ) ) {
         ...
         #     </graphics>
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; VM: [$say_vm]'s definition file: [$conf->{vm}{$vm}{definition_file}] is not using VNC, updating it via: [$node].\n"
                               );
            print AN::Common::template( $conf, "server.html",
                                        "switch-to-vnc-header" );
            my ($backup_file)
                = archive_file( $conf, $node, $conf->{vm}{$vm}{definition_file},
                                0, "hidden_table" );
            switch_vm_xml_to_vnc( $conf, $node, $vm, $backup_file );
            print AN::Common::template( $conf, "server.html",
                                        "switch-to-vnc-footer" );
        }

        AN::Cluster::record( $conf,
                             "$THIS_FILE " . __LINE__ . "; type: [$type]\n" );
        if ( $type ne "vnc" ) {
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; VM: [$say_vm] is not using VNC, can't use Guacamole.\n"
                               );
            ...
            $remote_icon =
                ...
                "image",
                {  image_source =>
                       "$conf->{url}{skins}/$conf->{sys}{skin}/images/icon_server-desktop_oops.png",
                   alt_text => "",
                   id       => "remote_icon",
                },
                "", 1 );
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; VM: [$say_vm] is using VNC, we can offer remote desktop access.\n"
                               );
            update_guacamole_config( $conf, $say_vm, $node, $port );
        }

        AN::Cluster::record( $conf,
                         "$THIS_FILE " . __LINE__ . "; message: [$message]\n" );
        if ( not $message ) {
            my ($guacamole_url)
                = AN::Cluster::get_guacamole_link( $conf, $node );
            ...
            if ( not $node ) {
                ...
                $remote_icon =
                    ...
                    "image",
                    {  image_source =>
                           "$conf->{url}{skins}/$conf->{sys}{skin}/images/icon_server-desktop_offline.png",
                       alt_text => "",
                       id       => "remote_icon",
                    },
                    "", 1 );
                ...
                my $image =
                    ...
                    "image",
                    {  image_source =>
                           "$conf->{url}{skins}/$conf->{sys}{skin}/images/icon_server-desktop_n01.png",
                       alt_text => "",
                ...
                $remote_icon =
                    AN::Common::template(
                             $conf,
                             "common.html",
                             "enabled-button-no-class-new-tab",
                ...
                my $image =
                    ...
                    "image",
                    {  image_source =>
                           "$conf->{url}{skins}/$conf->{sys}{skin}/images/icon_server-desktop_n02.png",
                       alt_text => "",
                ...
                $message = "#!string!message_0079!#"
                    ;    # Think the server is running, but unknown where.
                $remote_icon =
                    AN::Common::template(
                    $conf,
                    "common.html",
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; Guacamole does not appear to be installed.\n"
                           );
        $message
            = "#!string!message_0080!#";    # Version doesn't support guacamole
        $remote_icon =
            AN::Common::template(
            $conf,
            "common.html",
    ...
    foreach my $device ( sort { $a cmp $b } keys %{ $conf->{vm}{$vm}{cdrom} } )
    {
        ...
        my $say_optical_drive =
            AN::Common::get_string( $conf,
                                    {  key       => "device_0003",
                                       variables => { drive_number => $i, } } );
        print AN::Common::template( $conf,
                                    "server.html",
                                    "manager-server-optical-drive",
    ...
    my $restart_tomcat =
        AN::Common::get_string(
                ...
                { key       => "message_0085",
                  variables => {
                       reset_tomcat_url =>
                           "?cluster=$conf->{cgi}{cluster}&task=restart_tomcat",
                  } } );
...
sub switch_vm_xml_to_vnc {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; switch_vm_xml_to_vnc(); node: [$node], vm: [$vm], backup_file: [$backup_file]\n"
                       );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; vm::${vm}::raw_xml: [$conf->{vm}{$vm}{raw_xml}]\n"
                       );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; << new_definition: [$new_definition]\n");
        ...
        if (    ( $new_definition !~ /^<domain/s )
            ...
            if ( length($new_definition) < 5 ) {
                ...
                $say_error =
                    AN::Common::get_string(
                        $conf,
                        { key       => "message_0338",
                          variables => { definition_file => $definition_file, }
                        } );
            }
            else {
                # Malformed
            ...
            $new_definition
                = AN::Cluster::convert_text_to_html( $conf, $new_definition );
            print AN::Common::template($conf,
                                       "common.html",
                                       "bad-definition-file",
                                       { error              => $say_error,
            ...
            if ( not $first_line_read ) {
                ...
                if ( $line !~ /^<domain/ ) {
                    ...
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                            . "; First line diesn't start with: '<domain', recovery required!\n"
                    );
                }
                $first_line_read = 1;
            }
        ...
        if ($recover) {
            my ( $error, $ssh_fh, $output )
                ...
                {  node     => $node,
                   ...
                   'close'  => 0,
                   shell_call =>
                       "cp -f $backup_file $definition_file && chmod 644 $definition_file",
                } );
            AN::Cluster::record( $conf,
                      "$THIS_FILE "
                    . __LINE__
...
sub update_guacamole_config {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; update_guacamole_config; say_vm: [$say_vm], node: [$node], port: [$port]\n"
                       );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE " . __LINE__ . "; Reading: [$sc]\n" );
    my $fh = IO::Handle->new();
    open( $fh, "<$sc" )
        or die "$THIS_FILE " . __LINE__ . "; Failed to read: [$sc]\n";
    while (<$fh>) {
        chomp;
        my $line = $_;
        $line =~ s/^\s+//;
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Difference found, rewrite needed.\n");
                    ...
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                            . "; New values; guacamole::vm::${this_vm}::host: [$conf->{guacamole}{vm}{$this_vm}{host}], guacamole::vm::${this_vm}::port: [$conf->{guacamole}{vm}{$this_vm}{port}]!\n"
                    );
                }
                else {
...
# See if this VM still exists on the cluster.
                my $exists = 0;
                foreach
                    my $existing_vm ( sort { $a cmp $b } keys %{ $conf->{vm} } )
                {
                    $existing_vm =~ s/^vm://;
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; exists: [$exists], this_vm: [$this_vm], say_vm: [$say_vm].\n");
                if ( ( not $exists ) && ( $this_vm ne $say_vm ) ) {
                    ...
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                            . "; Existing server: [$this_vm] no longer exists, deleting it from guacamole.\n"
                    );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; This server not found, rewrite needed to add it.\n");
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; New values; guacamole::vm::${say_vm}::host: [$conf->{guacamole}{vm}{$say_vm}{host}], guacamole::vm::${say_vm}::port: [$conf->{guacamole}{vm}{$say_vm}{port}].\n"
                           );
    }
    ...
    if ($rewrite_needed) {
        ...
        my $say_warning
            = AN::Common::get_string( $conf, { key => "text_0006" } );
        my $say_updated =
            AN::Common::get_string( $conf,
                                    {  key       => "text_0007",
                                       variables => { date => $date, } } );
        ...
        foreach
            my $this_vm ( sort { $a cmp $b } keys %{ $conf->{guacamole}{vm} } )
        {
            ...
            $new_config
                .= "		<param name=\"hostname\" value=\"$this_host\" />\n";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; new guacamole config:\n===\n$new_config\n===\n");
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; Guacamole config update is not needed.\n"
                           );
    }
...
sub get_current_vm_vnc_info {
    my ( $conf, $vm, $node ) = @_;
    AN::Cluster::record( $conf,
        "$THIS_FILE " . __LINE__ . "; get_current_vm_vnc_info(); vm: [$vm]\n" );
    my $say_vm = $vm;
    if ( $vm =~ /^vm:/ ) {
        ($say_vm) = ( $vm =~ /^vm:(.*)/ );
    }
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE " . __LINE__ . "; say_vm: [$say_vm]\n" );
    ...
    if ( $conf->{vm}{$vm}{current_host} ) {
        ...
        # The VM isn't running.
        AN::Cluster::record( $conf,
               "$THIS_FILE " . __LINE__ . "; vm: [$say_vm] is not running.\n" );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$say_vm] definition line: [$line].\n");
        if ( $line =~ /^<graphics / ) {
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; vm: [$say_vm] is using: [$type] and listening on: [$listen] port: [$port].\n"
                               );
            last;
        }
    }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$say_vm], xml line: [$line]\n");
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; vm: [$say_vm], current_lv: [$current_lv], size: [$lv_size], on_vg: [$on_vg]\n"
                       );
...
sub update_vm_definition {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; in update_vm_definition(); node: [$node], vm: [$vm], say_vm: [$say_vm], definition_file: [$definition_file]\n"
                       );
    ...
    my ( $error, $ssh_fh, $output )
        = AN::Cluster::remote_call(
              ...
              { node     => $node,
                ...
                'close'  => 1,
                shell_call =>
                    "virsh dumpxml $say_vm > $definition_file; echo virsh:\$?",
              } );
    AN::Cluster::record(
                $conf,
                "$THIS_FILE "
    ...
    AN::Cluster::record( $conf,
         "$THIS_FILE " . __LINE__ . "; virsh exit code: [$virsh_exit_code]\n" );
    if ( $virsh_exit_code eq "0" ) {
        ...
        print AN::Common::template( $conf,
                                    "server.html",
                                    "update-definition-failed-bad-exit-code",
                                    { error => $say_error, } );
...
sub add_vm_to_cluster {
    my ($conf) = @_;
    AN::Cluster::record( $conf,
                         "$THIS_FILE " . __LINE__ . "; add_vm_to_cluster()\n" );
    ...
    AN::Cluster::record( $conf,
                "$THIS_FILE " . __LINE__ . "; node: [$node], peer: [$peer]\n" );
    my $failover_domain;
    ...
    AN::Cluster::scan_cluster($conf);
    AN::Cluster::record( $conf,
                    "$THIS_FILE " . __LINE__ . "; finished scan of Anvil!.\n" );
    foreach my $fod ( keys %{ $conf->{failoverdomain} } ) {
        AN::Cluster::record( $conf,
                             "$THIS_FILE " . __LINE__ . "; fod: [$fod]\n" );
        if ( $fod =~ /primary_(.*?)$/ ) {
            my $node_suffix = $1;
            AN::Cluster::record( $conf,
                 "$THIS_FILE " . __LINE__ . "; node_suffix: [$node_suffix]\n" );
            if ( $node =~ /$node_suffix/ ) {
                $failover_domain = $fod;
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; Using failover domain: [$failover_domain]\n"
                       );
    ...
    if ( not $conf->{clusters}{$cluster}{ricci_pw} ) {
        ...
        print AN::Common::template(
                                 $conf,
                                 "server.html",
                                 "add-server-to-anvil-no-ricci-password",
                                 {},
                                 { server     => $vm,
    ...
    if ( not $failover_domain ) {
        ...
        print AN::Common::template( $conf, "server.html",
                                    "add-server-to-anvil-no-failover-domain" );
        return (1);
    }
    ...
    if ( $host eq "none" ) {
        ...
        AN::Cluster::record( $conf,
                     "$THIS_FILE " . __LINE__ . "; I will now boot the VM.\n" );
        my $virsh_exit_code;
        my ( $error, $ssh_fh, $output )
            = AN::Cluster::remote_call(
                             $conf,
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; virsh exit code: [$virsh_exit_code]\n" );
        if ( $virsh_exit_code eq "0" ) {
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; It didn't start on the first try. Trying again with the definition file.\n"
                               );
            my $virsh_exit_code;
            ( $error, $ssh_fh, $output )
                = AN::Cluster::remote_call(
                ...
                {  node     => $node,
                   ...
                   'close'  => 1,
                   shell_call =>
                       "virsh create /shared/definitions/${vm}.xml; echo virsh:\$?",
                } );
            AN::Cluster::record( $conf,
                      "$THIS_FILE "
                    . __LINE__
            ...
            if ( $virsh_exit_code eq "0" ) {
                ...
                print AN::Common::template( $conf, "server.html",
                                           "add-server-to-anvil-failed-to-boot",
                                           { error => $say_error, } );
                return (1);
            }
        }
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; The VM is now running on the peer. Will proceed using: [$node].\n"
                           );
    }

    # Dump the VM's XML definition.
    print AN::Common::template( $conf, "server.html",
                                "add-server-to-anvil-write-definition" );
    ...
    if ( not $vm ) {
        ...
        print AN::Common::template( $conf,
                                    "server.html",
                                    "add-server-to-anvil-no-server-name",
    ...
    if ( $virsh_exit_code eq "0" ) {
        ...
        print AN::Common::template(
                            $conf, "server.html",
                            "add-server-to-anvil-failed-to-write-definition",
                            { error => $say_error, } );
        return (1);
    }
    ...
    # Undefine the new VM
    print AN::Common::template( $conf, "server.html",
                                "add-server-to-anvil-undefine-server" );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; virsh exit code: [$virsh_exit_code], undefine_ok: [$undefine_ok]\n"
                       );
    ...
    if ( $virsh_exit_code eq "0" ) {
        ...
        print AN::Common::template(
                             $conf, "server.html",
                             "add-server-to-anvil-failed-to-undefine-server",
                             { error => $say_error, } );
    }

  # If I've made it this far, I am ready to add it to the cluster configuration.
    print AN::Common::template( $conf, "server.html",
                                "add-server-to-anvil-add-to-rgmanager" );

    my $ccs_exit_code;
    my $ccs_call = "ccs ";
    $ccs_call
        .= "-h localhost --activate --sync --password \"$conf->{clusters}{$cluster}{ricci_pw}\" --addvm $vm ";
    ...
    $ccs_call .= "; echo ccs:\$?";
    AN::Cluster::record( $conf,
                        "$THIS_FILE " . __LINE__ . "; Calling: [$ccs_call]\n" );
    ( $error, $ssh_fh, $output )
        = AN::Cluster::remote_call(
                                $conf,
                                { node     => $node,
    ...
    foreach my $line ( @{$output} ) {
        ...
        if ( $line =~ /ccs:(\d+)/ ) {
            ...
            if ( $line =~ /make sure the ricci server is started/ ) {
                ...
                print AN::Common::template(
                    $conf, "server.html", "one-line-message",
    ...
    AN::Cluster::record( $conf,
             "$THIS_FILE " . __LINE__ . "; ccs exit code: [$ccs_exit_code]\n" );
    ...
    if ( $ccs_exit_code eq "0" ) {
        ...
        print AN::Common::template( $conf,
                                    "server.html",
                                    "add-server-to-anvil-ccs-call-failed",
                                    { error => $say_error, } );
        return (1);
    }
    ...
    print AN::Common::template( $conf, "server.html",
                                "add-server-to-anvil-enable-service" );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; Calling: [clusvcadm -e vm:$vm; echo clusvcadm:\$?]\n"
                       );
    ( $error, $ssh_fh, $output )
        = AN::Cluster::remote_call(
                     $conf,
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; clusvcadm exit code: [$clusvcadm_exit_code]\n"
                       );
    if ( $clusvcadm_exit_code eq "0" ) {
        ...
        print AN::Common::template(
                                $conf,
                                "server.html",
                                "add-server-to-anvil-clusvcadm-call-failed",
                                { error      => $say_error, },
                                { manage_url => "?config=true&anvil=$cluster", }
        );
        return (1);
...
sub find_vm_host {
    ...
    foreach my $line ( @{$output} ) {
        ...
        if ( $line =~ /\s$vm\s/ ) {
            AN::Cluster::record( $conf,
                               "$THIS_FILE " . __LINE__ . "; Found the VM.\n" );
            if ( $line =~ /^-/ ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; The VM appears to be off. Checking the peer to see if it is running there.\n"
                                   );
            }
            else {
                # It's running.
                $host = $node;
                AN::Cluster::record( $conf,
                          "$THIS_FILE " . __LINE__ . "; The VM is running.\n" );
    ...
    if ( $host eq "none" ) {
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; Looking for the VM on the peer node: [$peer].\n"
                           );
        my $on_peer = 0;
        my $found   = 0;
        my ( $error, $ssh_fh, $output )
    ...
    #print "Found it on the peer node. Checking if it's running there.<br />\n";
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; Found it on the peer. Checking if it's running.\n"
                                   );
                if ( $line =~ /^\d/ ) {
                    $found   = 1;
                    $on_peer = 1;
        ...
        if ( ($found) && ( not $on_peer ) ) {
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; I did not find it on the peer node.\n"
                               );
        }
    }
...
sub provision_vm {
    ...
    foreach my $vg ( keys %{ $conf->{new_vm}{vg} } ) {
        if ( lc( $conf->{new_vm}{vg}{$vg}{lvcreate_size} ) eq "all" ) {
            ...
            $provision
                .= "lvcreate -l 100\%FREE -n $conf->{new_vm}{name}_$i $vg\n";
        }
        else {
            $provision .= "lvremove -f /dev/$vg/$conf->{new_vm}{name}_$i\n";
            $provision
                .= "lvcreate -L $conf->{new_vm}{vg}{$vg}{lvcreate_size}GiB -n $conf->{new_vm}{name}_$i $vg\n";
        }
        $i++;
    }
    ...
    $provision
        .= "  --cdrom '/shared/files/$conf->{new_vm}{install_iso}' \\\\\n";
    if ( $conf->{cgi}{driver_iso} ) {
        $provision
            .= "  --disk path='/shared/files/$conf->{new_vm}{driver_iso}',device=cdrom --force\\\\\n";
    }
    ...
    # use 'cirrus'.
    if (    ( $conf->{cgi}{os_variant} ne "vista" )
         && ( $conf->{cgi}{os_variant} !~ /^win/ ) ) {
    ...
    foreach my $vg ( keys %{ $conf->{new_vm}{vg} } ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; LV: [$path], use virtio: [$conf->{new_vm}{virtio}{disk}]\n"
                           );
...
#$provision .= "  --graphics vnc --noautoconsole --wait -1 > /var/log/an-install_".$conf->{new_vm}{name}.".log &\n";
    $provision
        .= "  --graphics vnc,listen=0.0.0.0 --noautoconsole --wait -1 > /var/log/an-install_"
        . $conf->{new_vm}{name}
        . ".log &\n";

#$provision .= "  --graphics vnc,listen=127.0.0.1 > /var/log/an-install_".$conf->{new_vm}{name}.".log &\n";
    AN::Cluster::record( $conf,
                      "$THIS_FILE " . __LINE__ . "; provision:\n$provision\n" );
    ...
    print AN::Common::template( $conf,
                                ...
                                { server  => $conf->{new_vm}{name}, } );
    AN::Cluster::record( $conf,
                  "$THIS_FILE " . __LINE__ . "; Calling; script: [$script]\n" );
    ( $error, $ssh_fh, $output )
        = AN::Cluster::remote_call(
                                $conf,
                                { node     => $node,
    ...
    if ( -e $conf->{path}{guacamole_config} ) {
        ...
        print AN::Common::template( $conf,
                                    "server.html",
                                    "provision-server-install-started",
                                    {  icon           => $icon,
                                       restart_tomcat => $restart_tomcat,
                                    } );
    }
        ...
        print AN::Common::template( $conf, "server.html",
                              "provision-server-install-started-no-guacamole" );
    }
    ...
    print AN::Common::template(
        $conf,
        "server.html",
        "provision-server-footer",
        {},
        {  url =>
               "?cluster=$conf->{cgi}{cluster}&task=add_vm&name=$conf->{new_vm}{name}&node=$conf->{new_vm}{host_node}",
        } );
    ...
    if ( $conf->{'system'}{up_nodes} ) {
        ...
        if ( $conf->{cgi}{name} ) {
            ...
            if ( $conf->{cgi}{name} =~ /\s/ ) {
                ...
                # Bad name
                my $say_row
                    = AN::Common::get_string( $conf, { key => "row_0102" } );
                my $say_message = AN::Common::get_string( $conf,
                                                    { key => "message_0127" } );
                ...
                if ( exists $conf->{vm}{$vm_key} ) {
                    ...
                    # Duplicate name
                    my $say_row = AN::Common::get_string( $conf,
                                                        { key => "row_0103" } );
                    my $say_message =
                        AN::Common::get_string(
                                              $conf,
            ...
            # Missing server name
            my $say_row
                = AN::Common::get_string( $conf, { key => "row_0104" } );
            my $say_message
                = AN::Common::get_string( $conf, { key => "message_0129" } );
            push @errors, "$say_row#!#$say_message";
        }
        ...
        if ( $conf->{cgi}{cpu_cores} =~ /\D/ ) {
            ...
            # Not a digit.
            my $say_row
                = AN::Common::get_string( $conf, { key => "row_0105" } );
            my $say_message =
                AN::Common::get_string(
                                    $conf,
                                    { key       => "message_0130",
            ...
            # Not enough cores
            my $say_row
                = AN::Common::get_string( $conf, { key => "row_0106" } );
            my $say_message =
                AN::Common::get_string(
                      $conf,
                      { key       => "message_0131",
        ...
        if ( $conf->{cgi}{ram} =~ /\D/ ) {
            ...
            my $say_row
                = AN::Common::get_string( $conf, { key => "row_0107" } );
            my $say_message =
                AN::Common::get_string( $conf,
                                        {  key => "message_0132",
                                           variables =>
        ...
        my $requested_ram =
            AN::Cluster::hr_to_bytes( $conf,
                                      ...
                                      $conf->{cgi}{ram_suffix} );
        my $diff = $conf->{resources}{total_ram} % ( 1024**3 );
        my $available_ram = $conf->{resources}{total_ram}
            - $diff - $conf->{'system'}{unusable_ram};
        if ( $requested_ram > $available_ram ) {

            # Requested too much RAM.
            my $say_free_ram
                = AN::Cluster::bytes_to_hr( $conf, $available_ram );
            my $say_requested
                = AN::Cluster::bytes_to_hr( $conf, $requested_ram );
            my $say_row
                = AN::Common::get_string( $conf, { key => "row_0108" } );
            my $say_message =
                AN::Common::get_string( $conf,
                                        { key       => "message_0133",
                                          variables => {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; host_node: [$conf->{new_vm}{host_node}]\n");
                ...
                # Conflicting Storage
                my $say_row
                    = AN::Common::get_string( $conf, { key => "row_0109" } );
                my $say_message = AN::Common::get_string( $conf,
                                                    { key => "message_0134" } );
                push @errors, "$say_row#!#$say_message";
            }
            ...
            if ( $conf->{cgi}{$vg_key} eq "all" ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; cgi::${vg_key}: [$conf->{cgi}{$vg_key}], cgi::${vg_suffix_key}: [$conf->{cgi}{$vg_suffix_key}]\n"
                                   );
                my $lv_size =
                    AN::Cluster::hr_to_bytes( $conf,
                                              $conf->{cgi}{$vg_key},
                                              $conf->{cgi}{$vg_suffix_key} );
                AN::Cluster::record( $conf,
                       "$THIS_FILE " . __LINE__ . "; > lv_size: [$lv_size]\n" );
        ...
        if ( $conf->{cgi}{install_iso} ) {
            ...
            if ( exists $conf->{files}{shared}{$file_name} ) {
                ...
                # Dude, where's my ISO?
                my $say_row
                    = AN::Common::get_string( $conf, { key => "row_0110" } );
                my $say_message = AN::Common::get_string( $conf,
                                                    { key => "message_0135" } );
                push @errors, "$say_row#!#$say_message";
            }
            ...
            my $say_row
                = AN::Common::get_string( $conf, { key => "row_0110" } );
            my $say_message
                = AN::Common::get_string( $conf, { key => "message_0136" } );
            push @errors, "$say_row#!#$say_message";
        }
        ...
        if ( $conf->{cgi}{driver_iso} ) {
            ...
            if ( exists $conf->{files}{shared}{$file_name} ) {
                ...
                my $say_row
                    = AN::Common::get_string( $conf, { key => "row_0111" } );
                my $say_message = AN::Common::get_string( $conf,
                                                    { key => "message_0137" } );
                push @errors, "$say_row#!#$say_message";
            }
        ...
        if ( $conf->{cgi}{os_variant} ) {
            ...
            if ( not $match ) {
                ...
                my $say_row
                    = AN::Common::get_string( $conf, { key => "row_0112" } );
                my $say_message = AN::Common::get_string( $conf,
                                                    { key => "message_0138" } );
                push @errors, "$say_row#!#$say_message";
            }
            ...
            my $say_row
                = AN::Common::get_string( $conf, { key => "row_0113" } );
            my $say_message
                = AN::Common::get_string( $conf, { key => "message_0139" } );
            push @errors, "$say_row#!#$say_message";
        }
...
sub confirm_provision_vm {
    ...
    AN::Cluster::record( $conf,
           "$THIS_FILE " . __LINE__ . "; read file list from node: [$node]\n" );
    return if not $node;
    ...
    my $select_cpu_cores =
        AN::Cluster::build_select( $conf, "cpu_cores", 0, 0, 60,
                                   $conf->{cgi}{cpu_cores}, $cpu_cores );
    foreach
        my $storage ( sort { $a cmp $b } split /,/, $conf->{cgi}{max_storage} )
    {
        ...
        $conf->{cgi}{$vg_suffix_key} = "GiB"
            if not $conf->{cgi}{$vg_suffix_key};
        my $select_vg_suffix =
            AN::Cluster::build_select( $conf,
                                       "$vg_suffix_key",
                                       0,
                                       0,
                                       60,
        ...
        if ( $space < ( 2**30 ) ) {
            ...
            $select_vg_suffix =
                AN::Cluster::build_select( $conf, "$vg_suffix_key", 0, 0, 60,
                                           $conf->{cgi}{$vg_suffix_key},
                                           [ "MiB", "GiB" ] );
            $conf->{cgi}{$vg_suffix_key} = "GiB"
                if not $conf->{cgi}{$vg_suffix_key};
    ...
    $say_selects .= AN::Common::template( $conf, "server.html",
                                        "provision-server-vg-list-hidden-input",
    ...
    my $select_install_iso =
        AN::Cluster::build_select( $conf, "install_iso", 1, 1, 300,
                                   $conf->{cgi}{install_iso}, $images );
    my $select_driver_iso =
        AN::Cluster::build_select( $conf, "driver_iso", 1, 1, 300,
                                   $conf->{cgi}{driver_iso}, $images );
    my $select_os_variant =
        AN::Cluster::build_select( $conf,
                                   "os_variant",
                                   1,
                                   0,
...
sub confirm_migrate_vm {
    ...
    my $migration_time_estimate
        = $conf->{cgi}{vm_ram} / 1073741824;    # Get # of GB.
...
sub start_vm {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; vm: [$vm], say_vm: [$say_vm], node: [$node], node_cluster_name: [$node_cluster_name]\n"
                       );
        ...
        foreach my $line ( @{$output} ) {
            ...
            if ( $line =~ /Success/i ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; vm: [$vm], vm_key: [$vm_key], node: [$node], vm::${vm_key}::current_host: [$conf->{vm}{$vm_key}{current_host}]\n"
                                   );
            }
                ...
                foreach my $line ( @{$output} ) {
                    ...
                    print AN::Common::template( $conf,
                                                "server.html",
                                                "start-server-shell-output",
                                                { status  => $status,
                                                  message => $message,
                                                } );
                }
                print AN::Common::template( $conf, "server.html",
                                           "start-server-failed-trying-again" );
                ( $error, $ssh_fh, $output )
                    = AN::Cluster::remote_call(
                             $conf,
                             { node     => $node,
                               ...
                               'close'  => 1,
                               shell_call =>
                                   "clusvcadm -e vm:$vm -m $node_cluster_name",
                             } );
                AN::Cluster::record( $conf,
                          "$THIS_FILE "
                        . __LINE__
        ...
        if ($show_remote_desktop_link) {
            ...
            if ( -e $conf->{path}{guacamole_config} ) {
                ...
                if ( $type ne "vnc" ) {
                    ...
                    # will need to power off and restart the server before
                    # this will work.
                    $remote_message = AN::Common::get_string( $conf,
                                                    { key => "message_0076" } );
                    $remote_icon =
                        AN::Common::template(
                        $conf,
         ...
         #     </graphics>
                        AN::Cluster::record( $conf,
                                  "$THIS_FILE "
                                . __LINE__
                                . "; VM: [$say_vm]'s definition file is not using VNC, updating it.\n"
                        );
                        my ($backup_file)
                            = archive_file( $conf, $node,
                                            $conf->{vm}{$vm}{definition_file},
                                            0, "hidden_table" );
                        switch_vm_xml_to_vnc( $conf, $node, $vm, $backup_file );
                    }
                }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; restart_tomcat: [$restart_tomcat]\n");
            print AN::Common::template( $conf,
                                        "server.html",
                                        "start-server-show-guacamole-link",
                                        {  remote_icon    => $remote_icon,
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], ready: [$ready]\n");
    if ($ready) {
        ...
        print AN::Common::template( $conf,
                                    "server.html",
                                    "migrate-server-target-not-ready",
                                    {  title   => $say_title,
                                       message => $say_message,
                                    } );
    }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; in dual_join(), cluster: [$cluster]\n");
    ...
    my @abort_reason;
    foreach
        my $node ( sort { $a cmp $b } @{ $conf->{clusters}{$cluster}{nodes} } )
    {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Parent process has PID: [$parent_pid]. Spawning a child process for each node.\n");
        ...
        foreach my $node ( sort { $a cmp $b }
            ...
            if ($pid) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; node: [$node], Spawned child with PID: [$pid].\n"
                                   );
            }
            else {
     ...
     # long the %pids hash has data.
            ...
            do {
                ...
                if ( $pid < 1 ) {
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                            . "; Parent process thinks all children are gone now as wait returned: [$pid]. Exiting loop.\n"
                    );
                }
                else {
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                            . "; Parent process told that child with PID: [$pid] has exited.\n"
                    );
                }
                ...
                $saw_reaped++;
                my $say_error = AN::Common::get_string( $conf,
                                                    { key => "message_0192" } );
        ...
        print AN::Common::template( $conf, "server.html",
                                    "dual-join-anvil-aborted-header",
                                    { title => $say_title, } );
        ...
        print AN::Common::template( $conf, "server.html",
                                    "dual-join-anvil-aborted-footer" );
    }
...
sub delete_vm {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; in delete_vm(), vm: [$vm], cluster: [$cluster]\n"
                       );
        ...
        my $sat_problem =
            AN::Common::get_string( $conf,
                                    {  key       => "message_0195",
                                       variables => { server => $say_vm, } } );
        $abort_reason
            = "<b>$sat_problem!</b><br />#!string!message_0194!#<br />#!string!explain_0035!#<br />#!string!brand_0011!#<br />";
    }
    ...
    if ($proceed) {
        ...
        my ( $error, $ssh_fh, $output )
            ...
            {  node     => $host,
               ...
               'close'  => 0,
               shell_call =>
                   "ccs -h localhost --activate --sync --password \"$conf->{clusters}{$cluster}{ricci_pw}\" --rmvm $conf->{cgi}{vm}; echo ccs:\$?",
            } );
        AN::Cluster::record(
                $conf,
                "$THIS_FILE "
        ...
        if ( $ccs_exit_code eq "0" ) {
            ...
            print AN::Common::template( $conf,
                                        "server.html",
                                        "delete-server-bad-exit-code",
                                        { error => $say_error, } );
        }
        print AN::Common::template( $conf, "server.html",
                                    "delete-server-start-footer" );
        ...
        if ( ($stop_vm) && ( $ccs_exit_code eq "0" ) ) {
            ...
            print AN::Common::template( $conf, "server.html",
                                        "delete-server-force-off-header" );
            ...
            if ( $virsh_exit_code eq "0" ) {
                ...
                my $say_error =
                    AN::Common::get_string(
                        $conf,
                        { key       => "message_0200",
                          variables => { virsh_exit_code => $virsh_exit_code, }
                        } );
                print AN::Common::template( $conf, "server.html",
                      "delete-server-bad-exit-code", { error => $say_error, } );
            }
            print AN::Common::template( $conf, "server.html",
                                        "delete-server-force-off-footer" );
        }
        ...
        if ($proceed) {
            ...
            # Free up the storage
            print AN::Common::template( $conf, "server.html",
                                        "delete-server-remove-lv-header" );
            foreach my $lv ( keys %{ $conf->{vm}{$vm}{node}{$node}{lv} } ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; lvremove exit code: [$lvremove_exit_code]\n"
                                   );
...
sub remove_vm_definition {
    ...
    if ( $file !~ /^\/shared\/definitions\/.*?\.xml/ ) {
        ...
        print AN::Common::template( $conf, "server.html",
                                   "remove-vm-definition-wrong-definition-file",
                                   {}, { file => $file, } );
        return (1);
    }
    ...
    AN::Cluster::record( $conf,
               "$THIS_FILE " . __LINE__ . "; ls exit code: [$ls_exit_code]\n" );
...
sub archive_file {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; archive_file(); node: [$node], file: [$file], quiet: [$quiet], table_type: [$table_type]\n"
                       );
        ...
        if ( $line =~ /cp:(\d+)/ ) {
            ...
            if ( not $header_printed ) {
                ...
                if ( $table_type eq "hidden_table" ) {
                    print AN::Common::template( $conf, "server.html",
                                             "one-line-message-header-hidden" );
                }
                else {
                    print AN::Common::template( $conf, "server.html",
                                                "one-line-message-header" );
                }
                $header_printed = 1;
            }
            $line = parse_text_line( $conf, $line );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; cp exit code: [$cp_exit_code]\n");
    if ( $cp_exit_code eq "0" ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; The file: [$file] has been archived as: [$destination].\n"
                           );
    }
    else {
        # Failure
...
sub poweroff_node {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; in poweroff_node(), node: [$node], proceed: [$proceed]\n"
                       );
    if ($proceed) {
        ...
        my ( $error, $ssh_fh, $output )
            ...
            {  node     => $node,
               ...
               'close'  => 1,
               shell_call =>
                   "poweroff && echo \"Power down initiated. Please return to the main page now.\"",
            } );
        AN::Cluster::record(
                $conf,
                "$THIS_FILE "
...
sub dual_boot {
        ...
        my $node ( sort { $a cmp $b } @{ $conf->{clusters}{$cluster}{nodes} } )
    {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node::${node}::is_on: [$conf->{node}{$node}{is_on}]\n"
                           );
    ...
    if ($proceed) {
        ...
        print AN::Common::template( $conf, "server.html",
                                    "dual-boot-aborted-header",
                                    { title => $say_title, } );
        foreach my $reason (@abort_reasons) {
            print AN::Common::template( $conf, "server.html",
                                        "dual-boot-abort-reason",
                                        { reason => $reason, } );
        }
        print AN::Common::template( $conf, "server.html",
                                    "dual-boot-aborted-footer" );
    }
...
sub poweron_node {
    ...
    my $node_cluster_name = $conf->{cgi}{node_cluster_name};
    AN::Cluster::record( $conf,
            "$THIS_FILE " . __LINE__ . "; in poweron_node(), node: [$node]\n" );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node: [$node], is on: [$conf->{node}{$node}{is_on}]\n"
                       );
    my $proceed = 0;
    ...
    AN::Cluster::record( $conf,
          "$THIS_FILE " . __LINE__ . "; node: [$node], proceed: [$proceed]\n" );
    if ($proceed) {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], power check command: [$conf->{node}{$node}{info}{power_check_command}]\n");
            my ( $error, $ssh_fh, $output )
                = AN::Cluster::remote_call(
                 ...
                 { node     => $peer,
                   ...
                   'close'  => 1,
                   shell_call =>
                       "$conf->{node}{$node}{info}{power_check_command} -o on",
                 } );
            AN::Cluster::record( $conf,
                      "$THIS_FILE "
                    . __LINE__
            ...
            if ( $conf->{node}{$node}{info}{power_check_command} ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; node: [$node], target host: [$target_host], power check command: [$conf->{node}{$node}{info}{power_check_command}].\n"
                                   );
                my ( $local_access, $target_ip )
                    = AN::Cluster::on_same_network( $conf, $target_host,
                                                    $node );
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; node: [$node], local access: [$local_access].\n"
                                   );
                if ($local_access) {
                    ...
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                            . "; This machine is not on the same network out of band management interface: [$target_host] for node: [$node], unable to check power state.\n"
                    );
                }
            }
            else {
                ...
                my $say_title =
                    AN::Common::get_string(
                      $conf,
                      { key       => "title_0066",
                        variables => { node_anvil_name => $node_cluster_name, }
                      } );
                my $say_message =
                    AN::Common::get_string(
                      $conf,
...
sub fence_node {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; in poweron_node(), node: [$node], peer: [$peer], cluster name: [$node_cluster_name]\n"
                       );
    ...
    if ($proceed) {
        if ( not $conf->{node}{$peer}{up} ) {
            ...
            foreach my $address ( $fence_string =~ /-a\s(.*?)\s/g ) {
                my ( $local_access, $target_ip )
                    = AN::Cluster::on_same_network( $conf, $address, $node );
                if ( not $local_access ) {
                    $proceed = 0;
                    my $reason =
                        AN::Common::get_string( $conf,
    ...
    if ($proceed) {
        ...
        foreach my $line ( $fence_string =~ /\d+:.*?;\./g ) {
            print AN::Common::template( $conf, "server.html",
                                        "fence-node-output-header" );
            my ( $method_num, $method_name, $command )
                = ( $line =~ /(\d+):(.*?): (.*?;)\./ );
            ...
            if ($off_success) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; Fencing using the '$method_name' method succeeded. Proceeding with unfence action.\n"
                                   );
                my $say_message =
                    AN::Common::get_string(
                                $conf,
                ...
                print AN::Common::template( $conf, "server.html",
                           "fence-node-message", { message => $say_message, } );
                $fence_success = 1;
            }
            else {
                # Fence failed!
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; Fencing using the '$method_name' method failed. Will try next method, if available.\n"
                                   );
                my $say_message =
                    AN::Common::get_string(
                                $conf,
            ...
            if ($on_success) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; Unfencing using the '$method_name' method succeeded. Fence operation a complete success!\n"
                                   );
                my $say_message =
                    AN::Common::get_string(
                                $conf,
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; Unfencing using the '$method_name' method failed. The core fence action was a success, but something went wrong and manual intervention may be required before the node can be returned to service. If another fence method remains, it will now be tried in hopes of assisting recovery.\n"
                                   );
                my $say_message =
                    AN::Common::get_string(
                                $conf,
        ...
        print AN::Common::template( $conf, "server.html",
                                    "fence-node-aborted-header",
                                    { title => $say_title, } );
        foreach my $reason (@abort_reason) {
            print AN::Common::template( $conf, "server.html",
                                        "fence-node-abort-reason",
                                        { reason => $reason, } );
        }
    }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; error: [$error], ssh_fh: [$ssh_fh], output: [$output (".@{$output}." lines)]\n");
        ...
        print AN::Common::template( $conf, "server.html",
                                    "withdraw-node-close-output" );
        if ($rgmanager_stop) {
            print AN::Common::template( $conf, "server.html",
                                     "withdraw-node-resource-manager-stopped" );
            my $cman_stop = 1;
            ( $error, $ssh_fh, $output )
                = AN::Cluster::remote_call(
                                $conf,
            ...
            if ( not $cman_stop ) {
                ...
                # Crap...
                print AN::Common::template( $conf, "server.html",
                                  "withdraw-node-membership-withdrawl-failed" );
                my $cman_start = 1;
                ( $error, $ssh_fh, $output )
                    = AN::Cluster::remote_call(
                                $conf,
                ...
                if ($cman_start) {
                    ...
                    # and we're back
                    print AN::Common::template( $conf, "server.html",
                            "withdraw-node-membership-restarted-successfully" );
                    ...
                    # Failed, call support.
                    print AN::Common::template( $conf, "server.html",
                                  "withdraw-node-membership-restarted-failed" );
                }
            }
        }
        else {
            # Recover rgmanager
            print AN::Common::template( $conf, "server.html",
                              "withdraw-node-resource-manager-failed-to-stop" );
            recover_rgmanager( $conf, $node );
        }
        print AN::Common::template( $conf, "server.html",
                                    "withdraw-node-footer" );
    }
    else {
        my $say_title =
            AN::Common::get_string($conf,
...
sub recover_rgmanager {
    ...
    print AN::Common::template( $conf, "server.html",
                                "recover-resource-manager-header" );
    ...
    print AN::Common::template( $conf, "server.html",
                                "recover-resource-manager-output-footer" );

    if ($rgmanager_start) {
        my $storage_service = $conf->{node}{$node}{info}{storage_name};
        print AN::Common::template( $conf, "server.html",
                                    "recover-resource-manager-successful" );
        if ($storage_service) {
            ...
            print AN::Common::template($conf,
                                       "server.html",
                                       "recover-resource-manager-check-storage",
                                       {},
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; node: [$node], rescanning the #!string!brand_0003!# in ten seconds.\n"
                               );
            sleep 10;
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; node: [$node], storage service: [$storage_service], storage state: [$storage_state]\n"
                               );
            if ( $storage_state =~ /Failed/i ) {
                my $say_failed =
                    AN::Common::get_string(
                        $conf,
                        { key       => "message_0253",
                          variables => { storage_service => $storage_service, }
                        } );
                my $say_cycle =
                    AN::Common::get_string(
                        $conf,
                ...
                print AN::Common::template(
                               $conf,
                               "server.html",
                               "recover-resource-manager-cycle-storage-service",
                               { failed => $say_failed,
                                 cycle  => $say_cycle,
                               } );
                ...
                my ( $error, $ssh_fh, $output )
                    = AN::Cluster::remote_call(
                          ...
                          { node     => $node,
                            ...
                            'close'  => 0,
                            shell_call =>
                                "$conf->{path}{clusvcadm} -d $storage_service",
                          } );
                AN::Cluster::record( $conf,
                          "$THIS_FILE "
                        . __LINE__
                ...
                print AN::Common::template( $conf, "server.html",
                                      "recover-resource-manager-close-output" );
                ...
                if ($storage_stop) {
                    ...
                    print AN::Common::template(
                        $conf, "server.html",
                        "recover-resource-manager-storage-service-stopped",
                        { stopped => $say_stopped, } );
                    ...
                    ( $error, $ssh_fh, $output )
                        = AN::Cluster::remote_call(
                          ...
                          { node     => $node,
                            ...
                            'close'  => 1,
                            shell_call =>
                                "$conf->{path}{clusvcadm} -e $storage_service",
                          } );
                    AN::Cluster::record( $conf,
                              "$THIS_FILE "
                            . __LINE__
                    ...
                    if ($storage_start) {
                        ...
                        print AN::Common::template(
                            $conf,
                            "server.html",
                            "recover-resource-manager-storage-service-recovered",
                        ...
                        print AN::Common::template(
                            $conf,
                            "server.html",
                            "recover-resource-manager-storage-service-unrecovered",
                            { failed => $say_failed, } );
                    }
                }
                else {
                    ...
                    print AN::Common::template(
                        $conf, "server.html",
                        "recover-resource-manager-failed-to-disable",
                        { failed => $say_failed, } );
                }
            }
            else {
                ...
                print AN::Common::template(
                    $conf, "server.html",
                    "recover-resource-manager-abort-storage-recovery",
                    { abort => $say_abort, } );
            }
        }
        else {
            ...
            print AN::Common::template( $conf, "server.html",
                                 "recover-resource-manager-cant-find-storage" );
        }
    }
    else {
        # Failed. :(
        print AN::Common::template( $conf, "server.html",
                                    "recover-resource-manager-failed" );
    }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; up nodes: [$conf->{'system'}{up_nodes}] ($up_nodes)\n");
        if ( $conf->{'system'}{up_nodes} > 0 ) {
            ...
            $no_access_panel =
                AN::Common::template( $conf,
                                      "server.html",
                                      "display-details-nodes-unreachable",
...
sub display_free_resources {
    ...
    my $free_resources_panel .= AN::Common::template( $conf, "server.html",
                                      "display-details-free-resources-header" );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vg: [$vg], size: [$conf->{resources}{vg}{$vg}{size}], used space: [$conf->{resources}{vg}{$vg}{used_space}], free space: [$conf->{resources}{vg}{$vg}{free_space}], pv name: [$conf->{resources}{vg}{$vg}{pv_name}]\n");
        ...
        if ( not $conf->{'system'}{clvmd_down} ) {
            $enough_storage = 1
                if $conf->{resources}{vg}{$vg}{free_space} > ( 2**30 );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; allocated_ram: [$allocated_ram], vm: [$vm], ram: [$conf->{vm}{$vm}{details}{ram}]\n");
    ...
    my $real_total_ram
        = AN::Cluster::bytes_to_hr( $conf, $conf->{resources}{total_ram} );
    ...
    my $diff = $conf->{resources}{total_ram} % ( 1024**3 );
    $conf->{resources}{total_ram} = $conf->{resources}{total_ram}
        - $diff - $conf->{'system'}{unusable_ram};
    $conf->{resources}{total_ram} = 0 if $conf->{resources}{total_ram} < 0;
    my $free_ram
        = $conf->{'system'}{gfs2_down}
        ? 0
        : $conf->{resources}{total_ram} - $allocated_ram;
    my $say_free_ram
        = $conf->{'system'}{gfs2_down}
        ? "--"
        : AN::Cluster::bytes_to_hr( $conf, $free_ram );
    my $say_total_ram
        = AN::Cluster::bytes_to_hr( $conf, $conf->{resources}{total_ram} );
    my $say_allocated_ram
        = $conf->{'system'}{gfs2_down}
        ? "--"
        : AN::Cluster::bytes_to_hr( $conf, $allocated_ram );
    my $say_vg_size
        = $conf->{'system'}{clvmd_down}
        ? "--"
        : AN::Cluster::bytes_to_hr( $conf, $vg_size[0] );
    my $say_vg_used
        = $conf->{'system'}{clvmd_down}
        ? "--"
        : AN::Cluster::bytes_to_hr( $conf, $vg_used[0] );
    my $say_vg_free
        = $conf->{'system'}{clvmd_down}
        ? "--"
        : AN::Cluster::bytes_to_hr( $conf, $vg_free[0] );
    my $say_vg      = $conf->{'system'}{clvmd_down} ? "--" : $vg[0];
    my $say_pv_name = $conf->{'system'}{clvmd_down} ? "--" : $pv_name[0];

    # Show the main info.
    $free_resources_panel .=
        AN::Common::template(
                          $conf,
                          "server.html",
                          "display-details-free-resources-entry",
        ...
        for ( my $i = 1; $i < @vg; $i++ ) {
            ...
            $free_resources_panel .=
                AN::Common::template(
                           $conf,
                           "server.html",
                           "display-details-free-resources-entry-extra-storage",
                           { vg          => $vg[$i],
                             pv_name     => $pv_name[$i],
                             say_vg_size => $say_vg_size,
                             say_vg_used => $say_vg_used,
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; in enough_storage: [$enough_storage], free_ram: [$free_ram], node1 cman: [$conf->{node}{$node1}{daemon}{cman}{exit_code}], node2 cman: [$conf->{node}{$node2}{daemon}{cman}{exit_code}]\n");
    if (    ( $conf->{node}{$node1}{daemon}{cman}{exit_code} eq "0" )
        ...
        if ( ($enough_storage) && ( $free_ram > 1073741824 ) ) {
            $say_bns =
                ...
                "enabled-button-no-class",
                {  button_link =>
                       "?cluster=$conf->{cgi}{cluster}&task=provision&max_ram=$free_ram&max_cores=$conf->{resources}{total_cores}&max_storage=$vg_link",
                   button_text => "#!string!button_0022!#",
                   id          => "provision",
                },
                "", 1 );
    ...
    $free_resources_panel .=
        AN::Common::template( $conf,
                              "server.html",
                              "display-details-bottom-button-bar",
                              {  say_bns => $say_bns,
                                 say_mc  => $say_mc,
                              } );
    $free_resources_panel .= AN::Common::template( $conf, "server.html",
                                                   "display-details-footer" );
...
sub display_vm_details {
    ...
    my $node1                = $conf->{'system'}{cluster}{node1_name};
    my $node2                = $conf->{'system'}{cluster}{node2_name};
    my $server_details_panel = AN::Common::template( $conf, "server.html",
                                              "display-server-details-header" );
    ...
    foreach my $vm ( sort { $a cmp $b } keys %{ $conf->{vm} } ) {
        ...
        my $say_ram
            = $conf->{'system'}{gfs2_down}
            ? "#!string!symbol_0011!#"
            : AN::Cluster::bytes_to_hr( $conf, $conf->{vm}{$vm}{details}{ram} );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], say_ram: [$say_ram], ram: [$conf->{vm}{$vm}{details}{ram}]\n");
        ...
        my $node = "--";
        my $say_net_host
            = "";    # Don't want anything printed when the server is down
        my $say_host = "--";
        if ($host) {
...
#record ($conf, "$THIS_FILE ".__LINE__."; lv: [$lv], size: [$conf->{vm}{$vm}{node}{$node}{lv}{$lv}{size}]\n");
            ...
            foreach my $current_bridge ( sort { $a cmp $b }
                ...
                push @mac,
                    uc(
                      $conf->{vm}{$vm}{details}{bridge}{$current_bridge}{mac} );
                push @type,
                    $conf->{vm}{$vm}{details}{bridge}{$current_bridge}{type};
            }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], host: [$conf->{vm}{$vm}{host}]\n");
            if ( $conf->{vm}{$vm}{host} ne "none" ) {
                ...
                $say_net_host =
                    AN::Common::template($conf,
                                         "server.html",
                                         "display-server-details-network-entry",
                                         { host   => $say_host,
                                           bridge => $bridge[0],
                                           device => $device[0],
                                         } );
        ...
        $conf->{vm}{$vm}{details}{cpu_count} = "#!string!symbol_0011!#"
            if $conf->{'system'}{gfs2_down};
        ...
        $conf->{vm}{$vm}{details}{cpu_count} = "--"
            if not defined $conf->{vm}{$vm}{details}{cpu_count};
        ...
        $server_details_panel .=
            AN::Common::template(
                           $conf,
                           "server.html",
                           "display-server-details-resources",
                           { say_vm    => $say_vm,
        ...
        if ( $loop_count > 0 ) {
            for ( my $i = 1; $loop_count > $i; $i++ ) {
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; vm: [$vm], lv_path[$i]: [$lv_path[$i]], lv_size[$i]: [$lv_size[$i]]n"
                                   );
                ...
                if ( $bridge[$i] ) {
                    ...
                    if ( $conf->{vm}{$vm}{host} ne "none" ) {
                        ...
                        $say_net_host =
                            AN::Common::template(
                                            $conf,
                                            "server.html",
                                            "display-server-details-entra-nics",
                                            { say_host => $say_host,
                                              bridge   => $bridge[$i],
                                              device   => $device[$i],
                                            } );
                    ...
                    $say_network
                        = "$say_net_host <span class=\"highlight_detail\">$type[$i]</span> / <span class=\"highlight_detail\">$mac[$i]</span>";
                }
                ...
                $server_details_panel .=
                    AN::Common::template($conf,
                                         "server.html",
                                         "display-server-details-entra-storage",
    ...
    $server_details_panel .= AN::Common::template( $conf, "server.html",
                                              "display-server-details-footer" );
...
sub check_node_daemons {
    ...
    if ( not $node ) {
        AN::Cluster::error(
            $conf,
            "I was asked to check the daemons for a node, but was not passed a node name. This is likely a program error.\n"
        );
    }
...
sub check_node_readiness {
    ...
    if ( not $node ) {
        AN::Cluster::error(
            $conf,
            "I was asked to check the node readiness to run the $vm server, but was not passed a node name. This is likely a program error.\n"
        );
    }
            ...
            if ( $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{active} ) {
                ...
                foreach my $res ( sort { $a cmp $b }
                    ...
                    my $cs = $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{drbd}{$res}
                        {connection_state};
                    my $ro = $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{drbd}{$res}
                        {role};
                    my $ds = $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{drbd}{$res}
                        {disk_state};
...
sub read_vm_definition {
    ...
    if ( not $vm ) {
        AN::Cluster::error(
            $conf,
            "I was asked to look at a server's definition file, but no server was specified.",
            1 );
    }
    my $say_vm = $vm;
    if ( $vm =~ /vm:(.*)/ ) {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], say_vm: [$say_vm]\n");
    $conf->{vm}{$vm}{definition_file} = ""
        if not defined $conf->{vm}{$vm}{definition_file};
    $conf->{vm}{$vm}{xml} = "" if not defined $conf->{vm}{$vm}{xml};
    AN::Cluster::record( $conf,
              "$THIS_FILE "
            . __LINE__
            . "; in read_vm_definition(); node: [$node], vm: [$vm], say_vm: [$say_vm], definition_file: [$conf->{vm}{$vm}{definition_file}], XML array? ["
            . ref( $conf->{vm}{$vm}{xml} )
            . "]\n" );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; vm: [$vm], XML: [$conf->{vm}{$vm}{xml}], def: [$conf->{vm}{$vm}{definition_file}]\n"
                       );
    if (    ( not ref( $conf->{vm}{$vm}{xml} ) eq "ARRAY" )
         && ( $conf->{vm}{$vm}{definition_file} ) ) {
        $conf->{vm}{$vm}{raw_xml} = [];
        $conf->{vm}{$vm}{xml}     = [];
        my ( $error, $ssh_fh, $output )
    ...
    if ( not $conf->{vm}{$vm}{xml} ) {
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node: [$node]; I was asked to look at: [$vm]'s definition file, it was not read or was not found.\n"
                           );
        return (0);
    }
    AN::Cluster::record( $conf,
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], ram: [$conf->{vm}{$vm}{details}{ram}]\n");
        ...
        if ( $line =~ /<memory unit='(.*?)'>(\d+)<\/memory>/ ) {
            ...
            my $units = $1;
            my $ram   = $2;
            $conf->{vm}{$vm}{details}{ram}
                = AN::Cluster::hr_to_bytes( $conf, $ram, $units, 1 );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], cpu count: [$conf->{vm}{$vm}{details}{cpu_count}], type: [$cpu_type]\n");
        ...
        if ( ( $line =~ /<interface/ ) && ( $line =~ /type='bridge'/ ) ) {
            ...
            $conf->{vm}{$vm}{details}{bridge}{$current_bridge}{device}
                = $current_device ? $current_device : "unknown";
            $conf->{vm}{$vm}{details}{bridge}{$current_bridge}{mac}
                = $current_mac_address ? $current_mac_address : "unknown";
            $conf->{vm}{$vm}{details}{bridge}{$current_bridge}{type}
                = $current_interface_type ? $current_interface_type : "unknown";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; The node: [$node] is down, skipping LV check for: [$lv] for VM: [$vm]\n");
    ...
    $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{active}
        = $conf->{node}{$node}{lvm}{lv}{$lv}{active};
    $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{size}
        = AN::Cluster::bytes_to_hr( $conf,
                               $conf->{node}{$node}{lvm}{lv}{$lv}{total_size} );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], VM: [$vm], LV: [$lv], active: [$conf->{node}{$node}{lvm}{lv}{$lv}{active}], size: [$conf->{vm}{$vm}{node}{$node}{lv}{$lv}{size}], on device(s): [$conf->{node}{$node}{lvm}{lv}{$lv}{on_devices}]\n");
    ...
    foreach
        my $device ( split /,/, $conf->{node}{$node}{lvm}{lv}{$lv}{on_devices} )
    {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], node: [$node], lv: [$lv], on_res: [$on_res]\n");
        ...
        $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{drbd}{$on_res}{connection_state}
            = $conf->{drbd}{$on_res}{node}{$node}{connection_state};
        $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{drbd}{$on_res}{role}
            = $conf->{drbd}{$on_res}{node}{$node}{role};
        $conf->{vm}{$vm}{node}{$node}{lv}{$lv}{drbd}{$on_res}{disk_state}
            = $conf->{drbd}{$on_res}{node}{$node}{disk_state};
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node 1: n[$node1] s[$conf->{node}{$node1}{info}{short_host_name}] l[$conf->{node}{$node1}{info}{host_name}], node 2: n[$node2] s[$conf->{node}{$node2}{info}{short_host_name}] l[$conf->{node}{$node2}{info}{host_name}]\n");
    ...
    foreach my $vm ( sort { $a cmp $b } keys %{ $conf->{vm} } ) {
        ...
        if ( $vm =~ /^vm:(.*)/ ) {
            ...
            AN::Cluster::error(
                $conf,
                "I was asked to check on a VM that didn't have the <span class=\"code\">vm:</span> prefix. I got the name: <span class=\"code\">$vm</span>. This is likely a programming error.\n"
            );
        }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], say_vm: [$say_vm]\n");
        ...
        $conf->{vm}{$vm}{say_node1}
            ...
            ? "<span class=\"highlight_warning\">Not Ready</span>"
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], host: [$conf->{vm}{$vm}{host}]\n");
        if (    ( not $conf->{vm}{$vm}{host} )
             && ( not $conf->{'system'}{ignore_missing_vm} ) ) {

            # Pull the host node and current state out of the hash.
            my $host_node = "";
            my $vm_state  = "";
            foreach
                my $node ( sort { $a cmp $b } keys %{ $conf->{vm}{$vm}{node} } )
            {
                $host_node = $node;
            ...
            my $say_error =
                ...
                {  key       => "message_0271",
                   variables => {
                       server => $say_vm,
                       url =>
                           "?cluster=$conf->{cgi}{cluster}&task=add_vm&name=$say_vm&node=$host_node&state=$vm_state",
                   } } );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], failover_domain: [$failover_domain]\n");
    ...
    foreach my $priority ( sort { $a cmp $b }
        ...
        $prefered_host
            = $conf->{failoverdomain}{$failover_domain}{priority}{$priority}
            {node};
...
sub set_node_names {
    ...
    $conf->{'system'}{cluster}{node1_name}
        = $conf->{clusters}{$this_cluster}{nodes}[0];
    $conf->{'system'}{cluster}{node2_name}
        = $conf->{clusters}{$this_cluster}{nodes}[1];
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node2: [$node2], node2_long: [$node2_long]\n");
    ...
    my $vm_state_and_control_panel =
        AN::Common::template(
                            $conf,
                            "server.html",
                            "display-server-state-and-control-header",
                            { anvil => $conf->{cgi}{cluster},
                              node1_short_host_name =>
                                  $conf->{node}{$node1}{info}{short_host_name},
                              node2_short_host_name =>
                                  $conf->{node}{$node2}{info}{short_host_name},
                            } );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], say vm: [$say_vm]\n");
        ...
        my $say_start_target
            ...
            : "--";
        $say_start_target =~ s/\..*?$//;
        my $start_target_long
            = $node1_long =~ /$say_start_target/
            ? $conf->{node}{$node1}{info}{host_name}
            : $conf->{node}{$node2}{info}{host_name};
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; say_start_target: [$say_start_target], vm::${vm}::boot_target: [$conf->{vm}{$vm}{boot_target}], start_target_long: [$start_target_long]\n");
        ...
        if ( $conf->{vm}{$vm}{boot_target} ) {
            $prefered_host
                = "<span class=\"highlight_ready\">$prefered_host</span>";
        }
        else {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; on_host: [$on_host], prefered_host: [$prefered_host]\n");
            if ( ( $on_host eq $prefered_host ) || ( $on_host eq "none" ) ) {
                $prefered_host
                    = "<span class=\"highlight_good\">$prefered_host</span>";
            }
            else {
                $prefered_host
                    = "<span class=\"highlight_warning\">$prefered_host</span>";
            }
...
#my $migrate_button = "<span class=\"disabled_button\">#!string!button_0024!#</span>";
        ...
        if ( $conf->{vm}{$vm}{can_migrate} ) {
            ...
            $migrate_button =
                ...
                "enabled-button-no-class",
                {  button_link =>
                       "?cluster=$conf->{cgi}{cluster}&vm=$say_vm&task=migrate_vm&target=$conf->{vm}{$vm}{migration_target}&vm_ram=$conf->{vm}{$vm}{details}{ram}&confirm=true",
...
#my $force_off_button = "<span class=\"disabled_button\">#!string!button_0027!#</span>";
        ...
        if ( $conf->{vm}{$vm}{can_stop} ) {
            $host_node
                = long_host_name_to_node_name( $conf, $conf->{vm}{$vm}{host} );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; vm: [$vm], host node: [$host_node], vm host: [$conf->{vm}{$vm}{host}]\n");
            $stop_button =
                ...
                "enabled-button-no-class",
                {  button_link =>
                       "?cluster=$conf->{cgi}{cluster}&task=stop_vm&vm=$say_vm&node=$host_node",
            ...
            $force_off_button =
                ...
                {  button_class => "highlight_dangerous",
                   button_link =>
                       "?cluster=$conf->{cgi}{cluster}&task=force_off_vm&vm=$say_vm&node=$host_node&host=$conf->{vm}{$vm}{host}",
                   button_text => "#!string!button_0027!#",
...
#my $start_button     = "<span class=\"disabled_button\">#!string!button_0029!#</span>";
        ...
        if ( $conf->{vm}{$vm}{boot_target} ) {
            $start_button =
                ...
                "enabled-button-no-class",
                {  button_link =>
                       "?cluster=$conf->{cgi}{cluster}&task=start_vm&vm=$say_vm&node=$start_target_name&node_cluster_name=$start_target_long&confirm=true",
                   button_text => "#!string!button_0029!#",
                   id          => "start_vm_$vm",
                },
                "", 1 );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node::${node1}::daemon::cman::exit_code: [$conf->{node}{$node1}{daemon}{cman}{exit_code}], node::${node2}::daemon::cman::exit_code: [$conf->{node}{$node2}{daemon}{cman}{exit_code}], prefered_host: [$prefered_host]\n");
        if (    ( $conf->{node}{$node1}{daemon}{cman}{exit_code} eq "0" )
            ...
            $say_delete_button =
                ...
                {  button_class => "highlight_dangerous",
                   button_link =>
                       "?cluster=$conf->{cgi}{cluster}&vm=$say_vm&task=delete_vm",
                   button_text => "#!string!button_0030!#",
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__." > say_n1: [$conf->{vm}{$vm}{say_node1}], say_n2: [$conf->{vm}{$vm}{say_node2}]\n");
        if ( $conf->{vm}{$vm}{node1_ready} == 2 ) {
            $conf->{vm}{$vm}{say_node1}
                = "<span class=\"highlight_good\">#!string!state_0003!#</span>";
        }
        elsif ( $conf->{vm}{$vm}{node1_ready} == 1 ) {
            $conf->{vm}{$vm}{say_node1}
                = "<span class=\"highlight_ready\">#!string!state_0009!#</span>";
        }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__." < say_n1: [$conf->{vm}{$vm}{say_node1}], say_n2: [$conf->{vm}{$vm}{say_node2}]\n");
        ...
        my $dual_join = (    ( $conf->{node}{$node1}{enable_join} )
                          && ( $conf->{node}{$node2}{enable_join} ) ) ? 1 : 0;
        my $say_vm_link =
            AN::Common::template(
                  $conf,
                  "common.html",
                  ...
                  { button_class => "fixed_width_button",
                    button_link =>
                        "?cluster=$conf->{cgi}{cluster}&vm=$vm&task=manage_vm",
                    button_text => "$say_vm",
                    id          => "manage_vm_$say_vm",
                  },
                  "", 1 );
        if ($dual_join) {
            my $say_vm_disabled_button
                = AN::Common::template( $conf,
                                        "common.html",
                                        "disabled-button",
                                        { button_text => "$say_vm", },
        ...
        $vm_state_and_control_panel .=
            AN::Common::template( $conf,
                                  "server.html",
                                  "display-server-details-entry",
                                  { vm_link   => $say_vm_link,
    ...
    $vm_state_and_control_panel .= AN::Common::template( $conf, "server.html",
                                    "display-server-state-and-control-footer" );
...
sub display_drbd_details {
    ...
    my $drbd_details_panel =
        AN::Common::template( $conf,
                              "server.html",
                              "display-replicated-storage-header",
                              {  say_node1 => $say_node1,
                                 say_node2 => $say_node2,
                              } );
    ...
    foreach my $res ( sort { $a cmp $b } keys %{ $conf->{drbd} } ) {
        ...
        if ( $conf->{node}{$node1}{up} ) {
            ...
            if ( $conf->{node}{$node1}{daemon}{drbd}{exit_code} eq "0" ) {
                ...
                $say_n1_dev = $conf->{drbd}{$res}{node}{$node1}{device}
                    if $conf->{drbd}{$res}{node}{$node1}{device};
                $say_n1_cs = $conf->{drbd}{$res}{node}{$node1}{connection_state}
                    if $conf->{drbd}{$res}{node}{$node1}{connection_state};
                $say_n1_ro = $conf->{drbd}{$res}{node}{$node1}{role}
                    if $conf->{drbd}{$res}{node}{$node1}{role};
                $say_n1_ds = $conf->{drbd}{$res}{node}{$node1}{disk_state}
                    if $conf->{drbd}{$res}{node}{$node1}{disk_state};
                if (
                     ( $conf->{drbd}{$res}{node}{$node1}{disk_state} eq
                       "Inconsistent" )
                     && ( $conf->{drbd}{$res}{node}{$node1}{resync_percent}
                          =~ /^\d/ )
                    ) {
                    $say_n1_ds
                        .= " <span class=\"subtle_text\" style=\"font-style: normal;\">($conf->{drbd}{$res}{node}{$node1}{resync_percent}%)</span>";
                }
            }
            else {
                # It is not, use the {res_file} values.
                $say_n1_dev
                    = $conf->{drbd}{$res}{node}{$node1}{res_file}{device}
                    if $conf->{drbd}{$res}{node}{$node1}{res_file}{device};
                $say_n1_cs
                    = $conf->{drbd}{$res}{node}{$node1}{res_file}
                    {connection_state}
                    if $conf->{drbd}{$res}{node}{$node1}{res_file}
                    {connection_state};
                $say_n1_ro = $conf->{drbd}{$res}{node}{$node1}{res_file}{role}
                    if $conf->{drbd}{$res}{node}{$node1}{res_file}{role};
                $say_n1_ds
                    = $conf->{drbd}{$res}{node}{$node1}{res_file}{disk_state}
                    if $conf->{drbd}{$res}{node}{$node1}{res_file}{disk_state};
            }
        }
        ...
        if ( $conf->{node}{$node2}{up} ) {
            ...
            if ( $conf->{node}{$node2}{daemon}{drbd}{exit_code} eq "0" ) {
                ...
                $say_n2_dev = $conf->{drbd}{$res}{node}{$node2}{device}
                    if $conf->{drbd}{$res}{node}{$node2}{device};
                $say_n2_cs = $conf->{drbd}{$res}{node}{$node2}{connection_state}
                    if $conf->{drbd}{$res}{node}{$node2}{connection_state};
                $say_n2_ro = $conf->{drbd}{$res}{node}{$node2}{role}
                    if $conf->{drbd}{$res}{node}{$node2}{role};
                $say_n2_ds = $conf->{drbd}{$res}{node}{$node2}{disk_state}
                    if $conf->{drbd}{$res}{node}{$node2}{disk_state};
                if (
                     ( $conf->{drbd}{$res}{node}{$node2}{disk_state} eq
                       "Inconsistent" )
                     && ( $conf->{drbd}{$res}{node}{$node2}{resync_percent}
                          =~ /^\d/ )
                    ) {
                    $say_n2_ds
                        .= " <span class=\"subtle_text\" style=\"font-style: normal;\">($conf->{drbd}{$res}{node}{$node2}{resync_percent}%)</span>";
                }
            }
            else {
                # It is not, use the {res_file} values.
                $say_n2_dev
                    = $conf->{drbd}{$res}{node}{$node2}{res_file}{device}
                    if $conf->{drbd}{$res}{node}{$node2}{res_file}{device};
                $say_n2_cs
                    = $conf->{drbd}{$res}{node}{$node2}{res_file}
                    {connection_state}
                    if $conf->{drbd}{$res}{node}{$node2}{res_file}
                    {connection_state};
                $say_n2_ro = $conf->{drbd}{$res}{node}{$node2}{res_file}{role}
                    if $conf->{drbd}{$res}{node}{$node2}{res_file}{role};
                $say_n2_ds
                    = $conf->{drbd}{$res}{node}{$node2}{res_file}{disk_state}
                    if $conf->{drbd}{$res}{node}{$node2}{res_file}{disk_state};
            }
        }
        ...
        $drbd_details_panel .=
            AN::Common::template( $conf,
                                  "server.html",
                                  "display-replicated-storage-entry",
                                  {  res         => $res,
    ...
    $drbd_details_panel .= AN::Common::template( $conf, "server.html",
                                          "display-replicated-storage-footer" );
...
sub display_gfs2_details {
    ...
    my $gfs2_details_panel =
        AN::Common::template( $conf,
                              "server.html",
                              "display-cluster-storage-header",
                              {  say_node1 => $say_node1,
                                 say_node2 => $say_node2,
                              } );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node2 - cman exit code: [$conf->{node}{$node2}{daemon}{cman}{exit_code}], gfs2 exit code: [$conf->{node}{$node2}{daemon}{gfs2}{exit_code}]\n");
    if (    ( $conf->{node}{$node1}{daemon}{cman}{exit_code} eq "0" )
         && ( $conf->{node}{$node1}{daemon}{gfs2}{exit_code} eq "0" )
         && ( ref( $conf->{node}{$node1}{gfs} ) eq "HASH" ) ) {
        $gfs2_hash = $conf->{node}{$node1}{gfs};
        $node      = $node1;

#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; using node1's gfs2 hash: [$gfs2_hash]\n");
    }
    elsif (    ( $conf->{node}{$node2}{daemon}{cman}{exit_code} eq "0" )
            && ( $conf->{node}{$node2}{daemon}{gfs2}{exit_code} eq "0" )
            && ( ref( $conf->{node}{$node2}{gfs} ) eq "HASH" ) ) {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; up nodes: [$conf->{'system'}{up_nodes}]\n");
        if ( $conf->{'system'}{up_nodes} == 1 ) {
            ...
            $gfs2_details_panel .= AN::Common::template( $conf, "server.html",
                                         "display-cluster-storage-not-online" );
        }
    }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node::${node1}::gfs::${mount_point}::mounted: [$conf->{node}{$node1}{gfs}{$mount_point}{mounted}], node::${node2}::gfs::${mount_point}::mounted: [$conf->{node}{$node2}{gfs}{$mount_point}{mounted}]\n");
            my $say_node1_mounted
                = $conf->{node}{$node1}{gfs}{$mount_point}{mounted}
                ? "<span class=\"highlight_good\">#!string!state_0010!#</span>"
                : "<span class=\"highlight_bad\">#!string!state_0011!#</span>";
            my $say_node2_mounted
                = $conf->{node}{$node2}{gfs}{$mount_point}{mounted}
                ? "<span class=\"highlight_good\">#!string!state_0010!#</span>"
                : "<span class=\"highlight_bad\">#!string!state_0011!#</span>";
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; say_node1_mounted: [$say_node1_mounted], say_node2_mounted: [$say_node2_mounted]\n");
            ...
            $conf->{node}{$node1}{gfs}{$mount_point}{total_size} = ""
                if not
                defined $conf->{node}{$node1}{gfs}{$mount_point}{total_size};
            $conf->{node}{$node2}{gfs}{$mount_point}{total_size} = ""
                if not
                defined $conf->{node}{$node2}{gfs}{$mount_point}{total_size};
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node2 total size: [$conf->{node}{$node2}{gfs}{$mount_point}{total_size}]\n");
            if ( $conf->{node}{$node1}{gfs}{$mount_point}{total_size} =~ /^\d/ )
            {
                $say_size
                    = $conf->{node}{$node1}{gfs}{$mount_point}{total_size};
                $say_used
                    = $conf->{node}{$node1}{gfs}{$mount_point}{used_space};
                $say_used_percent
                    = $conf->{node}{$node1}{gfs}{$mount_point}{percent_used};
                $say_free
                    = $conf->{node}{$node1}{gfs}{$mount_point}{free_space};
            }
            elsif (
                 $conf->{node}{$node2}{gfs}{$mount_point}{total_size} =~ /^\d/ )
            {
                $say_size
                    = $conf->{node}{$node2}{gfs}{$mount_point}{total_size};
                $say_used
                    = $conf->{node}{$node2}{gfs}{$mount_point}{used_space};
                $say_used_percent
                    = $conf->{node}{$node2}{gfs}{$mount_point}{percent_used};
                $say_free
                    = $conf->{node}{$node2}{gfs}{$mount_point}{free_space};
            }
            $gfs2_details_panel .=
                AN::Common::template($conf,
                                     "server.html",
                                     "display-cluster-storage-entry",
        ...
        # No gfs2 FSes found
        $gfs2_details_panel .= AN::Common::template( $conf, "server.html",
                                   "display-cluster-storage-no-entries-found" );
    }
    $gfs2_details_panel .= AN::Common::template( $conf, "server.html",
                                             "display-cluster-storage-footer" );
...
sub display_node_details {
    ...
    foreach my $node ( sort { $a cmp $b }
                       @{ $conf->{clusters}{$this_cluster}{nodes} } ) {

        # Get the cluster's node name.
        my $say_short_name = $node;
        $say_short_name =~ s/\..*//;
        my $node_long_name
            = $node1_long =~ /$say_short_name/
            ? $conf->{node}{$node1}{info}{host_name}
            : $conf->{node}{$node2}{info}{host_name};
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node1_long: [$node1_long], node2_long: [$node2_long]\n");
    my $rowspan = 2;
    my $dual_boot = (    ( $conf->{node}{$node1}{enable_poweron} )
                      && ( $conf->{node}{$node2}{enable_poweron} ) ) ? 1 : 0;
    my $dual_join = (    ( $conf->{node}{$node1}{enable_join} )
                      && ( $conf->{node}{$node2}{enable_join} ) ) ? 1 : 0;
    foreach my $node ( sort { $a cmp $b }
                       @{ $conf->{clusters}{$this_cluster}{nodes} } ) {
        ...
        $conf->{node}{$node}{enable_withdraw} = 0
            if not defined $conf->{node}{$node}{enable_withdraw};

        # Join button.
        my $say_join_disabled_button =
            AN::Common::template( $conf,
                                  "common.html",
                                  "disabled-button",
        ...
        my $say_join_enabled_button =
            ...
            {  button_class => "bold_button",
               button_link =>
                   "?cluster=$conf->{cgi}{cluster}&task=join_cluster&node=$node&node_cluster_name=$node_long_name",
               button_text => "#!string!button_0031!#",
               id          => "join_cluster_$node",
            },
            "", 1 );
        $say_join[$i]
            = $conf->{node}{$node}{enable_join}
            ? $say_join_enabled_button
            : $say_join_disabled_button;
        $say_join[$i] = $say_join_disabled_button if $disable_join;

        # Withdraw button
        my $say_withdraw_disabled_button
            = AN::Common::template($conf,
                                   "common.html",
                                   "disabled-button",
                                   ...
                                   "",
                                   1 );
        my $say_withdraw_enabled_button
            = AN::Common::template(
            $conf,
            "common.html",
            "enabled-button-no-class",
            {  button_link =>
                   "?cluster=$conf->{cgi}{cluster}&task=withdraw&node=$node&node_cluster_name=$node_long_name",
               button_text => "#!string!button_0032!#",
               id          => "withdraw_$node",
            },
            "", 1 );
        $say_withdraw[$i]
            = $conf->{node}{$node}{enable_withdraw}
            ? $say_withdraw_enabled_button
            : $say_withdraw_disabled_button;

        # Shutdown button
        my $say_shutdown_disabled_button
            = AN::Common::template($conf,
                                   "common.html",
                                   "disabled-button",
                                   { button_text => "#!string!button_0033!#", },
                                   "",
                                   1 );
        my $say_shutdown_enabled_button
            = AN::Common::template(
            $conf,
            "common.html",
            "enabled-button-no-class",
            {  button_link =>
                   "?cluster=$conf->{cgi}{cluster}&task=poweroff_node&node=$node&node_cluster_name=$node_long_name",
               button_text => "#!string!button_0033!#",
               id          => "poweroff_node_$node",
            },
            "", 1 );
        $say_shutdown[$i]
            = $conf->{node}{$node}{enable_poweroff}
            ? $say_shutdown_enabled_button
            : $say_shutdown_disabled_button;

        # Boot button
        my $say_boot_disabled_button =
            AN::Common::template( $conf,
                                  "common.html",
                                  "disabled-button",
        ...
        my $say_boot_enabled_button =
            ...
            {  button_class => "bold_button",
               button_link =>
                   "?cluster=$conf->{cgi}{cluster}&task=poweron_node&node=$node&node_cluster_name=$node_long_name&confirm=true",
               button_text => "#!string!button_0034!#",
               id          => "poweron_node_$node",
            },
            "", 1 );
        $say_boot[$i]
            = $conf->{node}{$node}{enable_poweron}
            ? $say_boot_enabled_button
            : $say_boot_disabled_button;

        # Fence button
        # If the node is already confirmed off, no need to fence.
        my $say_fence_node_disabled_button
            = AN::Common::template($conf,
                                   "common.html",
                                   "disabled-button",
                                   ...
                                   "",
                                   1 );
        my $say_fence_node_enabled_button
            = AN::Common::template(
            $conf,
            "common.html",
            "enabled-button",
            {  button_class => "highlight_dangerous",
               button_link =>
                   "?cluster=$conf->{cgi}{cluster}&task=fence_node&node=$node&node_cluster_name=$node_long_name",
               button_text => "#!string!button_0037!#",
               id          => "fence_node_$node",
            },
            "", 1 );
        $say_fence[$i]
            = $conf->{node}{$node}{enable_poweron}
            ? $say_fence_node_disabled_button
            : $say_fence_node_enabled_button;

        # Dual-boot button.
        if ( $i == 0 ) {
            my $say_dual_boot_disabled_button
                = AN::Common::template(
                                   $conf,
                                   "common.html",
                                   "disabled-button",
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; say_dual_boot: [$say_dual_boot].\n");
            ...
            # Dual-Join button
            my $say_dual_join_disabled_button
                = AN::Common::template(
                                   $conf,
                                   "common.html",
                                   "disabled-button",
            ...
            if ($rowspan) {
                ...
                if ($dual_boot) {
                    $say_dual_boot =
                        ...
                        {  button_class => "bold_button",
                           button_link =>
                               "?cluster=$conf->{cgi}{cluster}&task=dual_boot&confirm=true",
                           button_text => "#!string!button_0035!#",
                           id          => "dual_boot",
                        },
                        "", 1 );
                ...
                if ($dual_join) {
                    $say_dual_join =
                        ...
                        {  button_class => "bold_button",
                           button_link =>
                               "?cluster=$conf->{cgi}{cluster}&task=dual_join&confirm=true",
                           button_text => "#!string!button_0036!#",
                           id          => "dual_join",
                        },
                        "", 1 );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; i: [$i], say_node_name: [$say_node_name[$i]].\n");
        if ( $conf->{node}{$node}{connected} ) {
            $say_node_name[$i]
                ...
                $conf,
                "common.html",
                "enabled-button-new-tab",
                {  button_class => "fixed_width_button",
                   button_link =>
                       "?cluster=$conf->{cgi}{cluster}&task=display_health&node=$node&node_cluster_name=$node_long_name",
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; say_dual_boot: [$say_dual_boot].\n");
    my $node_control_panel =
        AN::Common::template( $conf,
                              "server.html",
                              "display-node-controls-full",
