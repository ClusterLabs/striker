...
sub test_ssh_fingerprint {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; test_ssh_fingerprint(); node: [$node]\n" );
    ...
    while (<$fh>) {
        ...
        if ( $line =~ /The authenticity of host/ ) {
            ...
            AN::Cluster::record( $conf,
                "$THIS_FILE " . __LINE__ . "; authenticity of host message\n" );
            my $message = get_string( $conf,
                                      {  key       => "message_0279",
                                         variables => { node => $node, } } );
...
#print template($conf, "common.html", "shell-output-header");
            ...
            AN::Cluster::record( $conf,
                       "$THIS_FILE " . __LINE__ . "; offending key message\n" );
            my $file = $1;
            my $line = $2;
            my $message = get_string( $conf,
                                      {  key       => "message_0358",
...
sub get_date_and_time {
    ...
    my $require_weekday
        = $variables->{require_weekday} ? $variables->{require_weekday} : 0;
    my $skip_weekends
        = $variables->{skip_weekends} ? $variables->{skip_weekends} : 0;
    my $use_24h
        = $variables->{use_24h} ? $variables->{use_24h} : $conf->{sys}{use_24h};
    ...
    if ( ($skip_weekends) && ($offset) ) {
        ...
        if ( $local_offset =~ /^-/ ) {
            ...
            my $seconds_passed_today
                = $time{sec} + ( $time{min} * 60 ) + ( $time{hour} * 60 * 60 );
            ...
            $local_offset_days++
                if $local_offset_second_over_day > $seconds_passed_today;
            ...
            my $local_offset_days = $local_offset / $day;
            $local_offset_days++
                if $local_offset_second_over_day > $seconds_left_in_today;
    ...
    my $date
        = $time{year}
        . $conf->{sys}{date_seperator}
        . $time{pad_mon}
        . $conf->{sys}{date_seperator}
        . $time{pad_mday};
    my $time
        = $time{pad_hour}
        . $conf->{sys}{time_seperator}
        . $time{pad_min}
        . $conf->{sys}{time_seperator}
        . $time{pad_sec}
        . $time{suffix};
...
sub get_string {
    ...
    my $key = $vars->{key};
    my $language
        = $vars->{language} ? $vars->{language} : $conf->{sys}{language};
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; key: [$key], language: [$language], variables: [$variables]\n");
    ...
    if ( not $key ) {
        hard_die(
            ...
            __LINE__,
            2,
            "No string key was passed into common.lib's 'get_string()' function.\n"
        );
    }
    if ( not $language ) {
        hard_die(
            ...
            __LINE__,
            3,
            "No language key was set when trying to build a string in common.lib's 'get_string()' function.\n"
        );
    }
        ...
        hard_die(
            ...
            __LINE__,
            4,
            "The language key: [$language] does not exist in the 'strings.xml' file.\n"
        );
    }
    ...
    if ( $conf->{string}{lang}{$language}{lang}{long_name} ) {
        $say_language
            = "$language ($conf->{string}{lang}{$language}{lang}{long_name})";
    ...
    if ( ($variables) && ( ref($variables) ne "HASH" ) ) {
        hard_die(
            ...
            __LINE__,
            5,
            "The 'variables' string passed into common.lib's 'get_string()' function is not a hash reference. The string's data is: [$variables].\n"
        );
    }
...
#print "$THIS_FILE ".__LINE__."; string::lang::${language}::key::${key}::content: [$conf->{string}{lang}{$language}{key}{$key}{content}]\n";
    if ( not exists $conf->{string}{lang}{$language}{key}{$key}{content} ) {
        ...
        my @keys = grep {/\A\d+\z/} keys %{ $conf->{agents} };
        my @xml_files;
        for my $key (@keys) {
            push @xml_files, $conf->{agents}{$key}{program} . '.xml';
        }
        my $xml_files = join ', ', @xml_files;
        ...
        hard_die(
            ...
            __LINE__,
            6,
            "The 'string' generated by common.lib's 'get_string()' function is undefined.<br />This passed string key: '$key' for the language: '$say_language' may not exist in any of the XML files: ($xml_files).\n"
        );
    }
...
sub initialize_conf {
    ...
    my $conf = {
        ...
        path             => {
            ...
            backup_config =>
                "/var/www/html/striker-backup_#!hostname!#_#!date!#.txt"
            , # Remember to update the sys::backup_url value below if you change this
            'call_gather-system-info' =>
                "/var/www/tools/call_gather-system-info",
            cat                 => "/bin/cat",
            ccs                 => "/usr/sbin/ccs",
            ...
            shared              => "/shared/files/"
            ,    # This is hard-coded in the file delete function.
            ...
            config_file =>
                "/etc/striker/striker.conf",    # TODO: Why is this here?!
            ...
            nodes => {
                  ...
                  hosts    => "/etc/hosts",
                  bcn_bond1_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-bcn-bond1",
                  bcn_link1_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-bcn-link1",
                  bcn_link2_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-bcn-link2",
                  ifcfg_directory => "/etc/sysconfig/network-scripts/",
                  ifn_bond1_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-ifn-bond1",
                  ifn_bridge1_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-ifn-bridge1",
                  ifn_link1_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-ifn-link1",
                  ifn_link2_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-ifn-link2",
                  sn_bond1_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-sn-bond1",
                  sn_link1_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-sn-link1",
                  sn_link2_config =>
                      "/etc/sysconfig/network-scripts/ifcfg-sn-link2",
                  udev_net_rules => "/etc/udev/rules.d/70-persistent-net.rules",
            },
        },
        args => {check_dvd =>
                     "--dvd --no-cddb --no-device-info --no-disc-mode --no-vcd",
                 rsync => "-av --partial",
                },
        sys => {
...
#print "$THIS_FILE ".__LINE__."; variable [$variable]: [$variables->{$variable}]\n";
        ...
        if ( $i > $conf->{sys}{error_limit} ) {
            hard_die(
                ...
                __LINE__,
                7,
                "Infitie loop detected will inserting variables into the string: [$string]. If this is triggered erroneously, increase the 'sys::error_limit' value.\n"
            );
        }
        $i++;
    }
    ...
    if ( $conf->{sys}{log_level} >= $level ) {
        ...
        if ( not $fh ) {
            ...
            open( $fh, ">>$log_file" )
                ...
                __LINE__,
                13,
                "Unable to open the file: [$log_file] for writing. The error was: $!.\n"
                );
            my ( $date, $time ) = get_date_and_time($conf);
            my $say_log_header =
                get_string( $conf,
                            {  language  => $conf->{sys}{log_language},
                               key       => "log_0001",
                               variables => { date   => $date,
                                              'time' => $time,
                                            } } );
...
sub template {
    my ( $conf, $file, $template, $replace, $variables, $hide_template_name )
        = @_;
    ...
    if ( not -e $template_file ) {
        hard_die( $conf, $THIS_FILE, __LINE__, 10,
            "The template file: [$template_file] does not appear to exist.\n" );
    }
        ...
        hard_die(
            ...
            __LINE__,
            11,
            "The template file: [$template_file] is not readable by the user this program is running as the user: [$user]. Please check the permissions on the template file and it's parent directory.\n"
        );
    }
    ...
    open( $read, $shell_call )
        or hard_die( $conf, $THIS_FILE, __LINE__, 1,
                     "Failed to read: [$shell_call]. The error was: $!\n" );
    binmode $read, ":utf8:";
    while (<$read>) {
        chomp;
        my $line = $_;
 ...
 #AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; >> string: [$string]\n");
        ($string)
            = process_string_replace( $conf, $string, $replace, $template_file,
                                      $template );
...
sub process_string {
 ...
 #AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; << string: [$string]\n");
    ...
    while ( $string =~ /#!(.+?)!#/s ) {
        ...
        ($string)
            = process_string_protect_escape_variables( $conf, $string,
                                                       "string" );
        ...
        if ( $i > $conf->{sys}{error_limit} ) {
            hard_die(
                ...
                __LINE__,
                8,
                "Infitie loop detected will processing escape variables in the string: [$string]. If this is triggered erroneously, increase the 'sys::error_limit' value. If you are a developer or translator, did you use '#!replace!...!#' when you meant to use '#!variable!...!#' in a string key?\n"
            );
        }
        $i++;
    }
...
sub process_string_insert_strings {
    ...
    while ( $string =~ /#!string!(.+?)!#/ ) {
        ...
        #print __LINE__."; >>> string: [$string]\n";
        my $say_string
            = get_string( $conf, { key => $key, variables => $variables } );
    ...
    while ( $string =~ /#!replace!(.+?)!#/ ) {
        ...
        if ( $i > $conf->{sys}{error_limit} ) {
            hard_die(
                ...
                __LINE__,
                12,
                "Infitie loop detected while replacing '#!replace!...!#' replacement variables in the template file: [$template_file] in the template: [$template]. If this is triggered erroneously, increase the 'sys::error_limit' value.\n"
            );
        }
        $i++;
    }
    ...
    while ( $string =~ /_!\|(.+?)\|!_/s ) {
        ...
        if ( $i > $conf->{sys}{error_limit} ) {
            hard_die(
                ...
                __LINE__,
                9,
                "Infitie loop detected will restoring protected escape variables in the string: [$string]. If this is triggered erroneously, increase the 'sys::error_limit' value.\n"
            );
        }
        $i++;
    }
...
sub read_strings {
    ...
    my $in_comment = 0
        ;   # Set to '1' when in a comment stanza that spans more than one line.
    my $in_data
        = 0;    # Set to '1' when reading data that spans more than one line.
    my $closing_key
        = "";    # While in_data, look for this key to know when we're done.
    my $xml_version = "";    # The XML version of the strings file.
    my $encoding
        = "";    # The encoding used in the strings file. Should only be UTF-8.
    my $data     = "";    # The data being read for the given key.
    my $key_name = ""
        ; # This is a double-colon list of hash keys used to build each hash element.

    my $read       = IO::Handle->new;
    my $shell_call = "<$file";
    open( $read, $shell_call )
        or die hard_die( $conf, $THIS_FILE, __LINE__, 1,
                         "Failed to read: [$shell_call]. The error was: $!\n" );
        ...
        if ( not $in_data ) {
            ...
            if ( ( $line =~ /<(.*?) (.*?)>/ ) && ( $line =~ /<\/$1>/ ) ) {
                ...
                my ( $name, $data )
                    = ( $line =~ /^<$key name="(.*?)">(.*?)<\/$key>/ );
                $data = $cdata if $cdata;
                _make_hash_reference( $string_ref,
                                      "${key_name}::${key}::${name}::content",
                                      $data );
                next;
            }
            ...
            if ( ( $line =~ /<(.*?)>/ ) && ( $line =~ /<\/$1>/ ) ) {
                ...
                $data = $line;
                $data =~ s/<$key>(.*?)<\/$key>/$1/;
                _make_hash_reference( $string_ref, "${key_name}::${key}",
                                      $data );
                next;
            }
            ...
            if ( $line !~ /<\/$closing_key>/ ) {
               ...
               #print "Saving: key: [$key], [${key_name}::${key}] -> [$data]\n";
                ...
                if ( $save_data =~ /\S/s ) {
                    ...
                    _make_hash_reference( $string_ref, "${key_name}::content",
                                          $save_data );
                }
...
sub get_screen_width {
    ...
    my $cols = 0;
    open my $fh, '-|', "$conf->{path}{tput}", "cols"
        or die "Failed to call: [$conf->{path}{tput} cols]\n";
    while (<$fh>) {
        chomp;
        $cols = $_;
    }
...
sub _get_hash_value_from_string {
    ...
    my $value
        = defined $this_href->{$last_key}
        ? $this_href->{$last_key}
        : "!!Undefined config variable: [$key_string]!!";
