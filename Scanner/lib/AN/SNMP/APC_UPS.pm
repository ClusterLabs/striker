...
sub process_all_oids {
	...
	### TODO: This is a dirty hack to fix the processing of "hours" as
	###       "minutes". It needs to be fixed properly.
	if ($tag eq "battery runtime remaining")
	{
		# Dirty fix for now
		#say "i: [$i], tag: [$tag], value: [$value], rec_meta: [$rec_meta], prev_status: [$prev_status], prev_value: [$prev_value]\n";
		if ($value =~ /(\d+) h.*?, (\d+):(\d+.\d+)/)
		{
			my $hours   = $1;
			my $minutes = $2;
