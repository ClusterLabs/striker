...
sub run_new_install_manifest {
    ...
    print AN::Common::template( $conf, "install-manifest.html",
                                "new-anvil-install-header" );
    ...
    $conf->{packages}{to_install} = {
        ...
        # similar) output.
        MegaCli => 0, };
    $conf->{url}{'anvil-map-network'}
        = "https://raw.githubusercontent.com/digimer/striker/master/tools/anvil-map-network";
    ...
    if ( $conf->{perform_install} ) {
        ...
        if ( $conf->{cgi}{update_manifest} ) {
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; >> cgi::run: [$conf->{cgi}{run}].\n"
                               );
            my ( $target_url, $xml_file )
                = AN::Cluster::generate_install_manifest($conf);
            print AN::Common::template(
                                $conf,
                                "install-manifest.html",
                                "manifest-created",
                                { message =>
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node1_remap_required: [$node1_remap_required], node2_remap_required: [$node2_remap_required].\n"
                       );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node1_rc: [$node1_rc], node2_rc: [$node2_rc].\n"
                       );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; cgi::perform_install: [$conf->{cgi}{perform_install}].\n"
                       );
    if ( not $conf->{cgi}{perform_install} ) {
        ...
        foreach my $node ( $conf->{cgi}{anvil_node1_current_ip},
                           $conf->{cgi}{anvil_node2_current_ip} ) {
...
sub register_with_rhn {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; cgi::rhn_user: [$conf->{cgi}{rhn_user}], cgi::rhn_password: [$conf->{cgi}{rhn_password}]\n"
                       );
    ...
    if ( (
           (  $conf->{node}{$node1}{os}{brand}
              =~ /Red Hat Enterprise Linux Server/ )
           && ( not $conf->{node}{$node1}{os}{registered} )
           && ( $conf->{node}{$node1}{internet} ) )
         || (
              ( $conf->{node}{$node2}{os}{brand}
                =~ /Red Hat Enterprise Linux Server/ )
              && ( not $conf->{node}{$node2}{os}{registered} )
              && ( $conf->{node}{$node2}{internet} ) )
        ) {
        print AN::Common::template( $conf,
                                    "install-manifest.html",
                                    "new-anvil-install-be-patient-message",
                                    { message => "#!string!explain_0138!#", } );
    }

    # If it's not RHEL, no sense going further.
    if (
        (  $conf->{node}{$node1}{os}{brand} !~ /Red Hat Enterprise Linux Server/
        )
        && ( $conf->{node}{$node2}{os}{brand}
             !~ /Red Hat Enterprise Linux Server/ )
        ) {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node::${node1}::os::brand: [$conf->{node}{$node1}{os}{brand}], node::${node2}::os::brand: [$conf->{node}{$node2}{os}{brand}]\n"
                       );
    if ( $conf->{node}{$node1}{os}{brand} =~ /Red Hat Enterprise Linux Server/ )
    {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node::${node1}::os::registered: [$conf->{node}{$node1}{os}{registered}], node::${node1}::internet: [$conf->{node}{$node1}{internet}]\n"
                           );
        if (    ( not $conf->{node}{$node1}{os}{registered} )
             && ( $conf->{node}{$node1}{internet} ) ) {
            ...
            AN::Cluster::record( $conf,
                       "$THIS_FILE " . __LINE__ . "; node1_ok: [$node1_ok]\n" );
        }
        else {
            $node1_ok = "skip";
            AN::Cluster::record( $conf,
    ...
    if ( $conf->{node}{$node2}{os}{brand} =~ /Red Hat Enterprise Linux Server/ )
    {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node::${node2}::os::registered: [$conf->{node}{$node2}{os}{registered}], node::${node2}::internet: [$conf->{node}{$node2}{internet}]\n"
                           );
        if (    ( not $conf->{node}{$node2}{os}{registered} )
             && ( $conf->{node}{$node2}{internet} ) ) {
            ...
            AN::Cluster::record( $conf,
                       "$THIS_FILE " . __LINE__ . "; node2_ok: [$node2_ok]\n" );
        }
        else {
            $node2_ok = "skip";
            AN::Cluster::record( $conf,
...
sub register_node_with_rhn {
    ...
    my $return_code       = 0;
    my $shell_call
        = "rhnreg_ks --username \"$conf->{cgi}{rhn_user}\" --password \"$conf->{cgi}{rhn_password}\" --force --profilename \"$name\" && ";
    $shell_call
        .= "rhn-channel --add --user \"$conf->{cgi}{rhn_user}\" --password \"$conf->{cgi}{rhn_password}\" --channel=rhel-x86_64-server-rs-6 && ";
    $shell_call
        .= "rhn-channel --add --user \"$conf->{cgi}{rhn_user}\" --password \"$conf->{cgi}{rhn_password}\" --channel=rhel-x86_64-server-optional-6 && ";
    $shell_call
        .= "rhn-channel --list --user \"$conf->{cgi}{rhn_user}\" --password \"$conf->{cgi}{rhn_password}\"";
    AN::Cluster::record( $conf,
                   "$THIS_FILE " . __LINE__ . "; shell_call: [$shell_call]\n" );
    my ( $error, $ssh_fh, $return )
        = AN::Cluster::remote_call( $conf,
                                    { node     => $node,
                                      port     => 22,
                                      user     => "root",
                                      password => $password,
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; error: [$error], ssh_fh: [$ssh_fh], return: [$return (".@{$return}." lines)]\n");
    foreach my $line ( @{$return} ) {
        AN::Cluster::record( $conf,
                             "$THIS_FILE " . __LINE__ . "; return: [$line]\n" );
    ...
    if ( ( not $base ) || ( not $resilient_storage ) || ($optional) ) {
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; Registration failed; node: [$node], base: [$base], resilient_storage: [$resilient_storage], optional: [$optional]\n"
                           );
        $return_code = 1;
    }
...
sub summarize_build_plan {
    ...
    my $say_node1_os
        ...
        : $conf->{node}{$node1}{os}{brand};
    my $say_node2_os
        = $conf->{node}{$node2}{os}{brand} =~ /Red Hat Enterprise Linux Server/
        ? "RHEL"
        : $conf->{node}{$node2}{os}{brand};
    my $rhn_template = "";
    if ($enable_rhn) {
        ...
        {  form_file => "/cgi-bin/striker",
           ...
           media_library_size =>
               AN::Cluster::bytes_to_hr(
                              $conf, $conf->{cgi}{anvil_media_library_byte_size}
               ),
           pool1_size =>
               AN::Cluster::bytes_to_hr(
           ...
           edit_manifest_url =>
               "?config=true&task=create-install-manifest&load=$conf->{cgi}{run}",
           remap_network_url =>
               "$conf->{'system'}{cgi_string}&remap_network=true",
...
sub configure_network_on_node {
    ...
    my $udev_rules
        = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf)
        . "].\n\n";
    $udev_rules .= "# Back-Channel Network, Link 1\n";
    $udev_rules
        .= "SUBSYSTEM==\"net\", ACTION==\"add\", DRIVERS==\"?*\", ATTR{address}==\"$conf->{cgi}{$bcn_link1_mac_key}\", NAME=\"bcn-link1\"\n\n";
    $udev_rules .= "# Back-Channel Network, Link 2\n";
    $udev_rules
        .= "SUBSYSTEM==\"net\", ACTION==\"add\", DRIVERS==\"?*\", ATTR{address}==\"$conf->{cgi}{$bcn_link2_mac_key}\", NAME=\"bcn-link2\"\n\n";
    $udev_rules .= "# Storage Network, Link 1\n";
    $udev_rules
        .= "SUBSYSTEM==\"net\", ACTION==\"add\", DRIVERS==\"?*\", ATTR{address}==\"$conf->{cgi}{$sn_link1_mac_key}\", NAME=\"sn-link1\"\n\n";
    $udev_rules .= "# Storage Network, Link 2\n";
    $udev_rules
        .= "SUBSYSTEM==\"net\", ACTION==\"add\", DRIVERS==\"?*\", ATTR{address}==\"$conf->{cgi}{$sn_link2_mac_key}\", NAME=\"sn-link2\"\n\n";
    $udev_rules .= "# Internet-Facing Network, Link 1\n";
    $udev_rules
        .= "SUBSYSTEM==\"net\", ACTION==\"add\", DRIVERS==\"?*\", ATTR{address}==\"$conf->{cgi}{$ifn_link1_mac_key}\", NAME=\"ifn-link1\"\n\n";
    $udev_rules .= "# Internet-Facing Network, Link 2\n";
    $udev_rules
        .= "SUBSYSTEM==\"net\", ACTION==\"add\", DRIVERS==\"?*\", ATTR{address}==\"$conf->{cgi}{$ifn_link2_mac_key}\", NAME=\"ifn-link2\"\n";

    ### Back-Channel Network
    #$conf->{path}{nodes}{bcn_link1_config};
    my $ifcfg_bcn_link1 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_bcn_link2 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_bcn_bond1 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_sn_link1 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_sn_link2 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_sn_bond1 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_ifn_link1 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_ifn_link2 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_ifn_bond1 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    my $ifcfg_ifn_bridge1 = "# Generated by: [$THIS_FILE] on: ["
        . AN::Cluster::get_date($conf) . "].\n";
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; Deleting any existing ifcfg-eth* files on: [$node]\n"
                       );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; error: [$error], ssh_fh: [$ssh_fh], return: [$return (".@{$return}." lines)]\n");
    ...
    my $hosts_body
        = $conf->{install_manifest}{$manifest}{common}{file}{$hosts_file}
        {content};
    if ($hosts_body) {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; error: [$error], ssh_fh: [$ssh_fh], return: [$return (".@{$return}." lines)]\n");
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; Hosts file content not found! node: [$node], manifest: [$manifest], hosts_file: [$hosts_file]\n"
                           );
    }
...
sub configure_network {
    ...
    my ($node1_ok)
        = configure_network_on_node( $conf,
                                     $conf->{cgi}{anvil_node1_current_ip},
                                     $conf->{cgi}{anvil_node1_current_password},
                                     1,
                                     "#!string!device_0005!#" );
...
sub parse_script_line {
    ...
    if ( $line =~ /#!exit!(.*?)!#/ ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node::${node}::reboot_needed: [$conf->{node}{$node}{reboot_needed}].\n"
                           );
        return ("<br />\n");
    }
    elsif ( $line =~ /#!string!(.*?)!#$/ ) {
...
sub map_network {
    ...
    $conf->{conf}{node}{$node2}{set_nic}{'ifn-link1'} = "";
    $conf->{conf}{node}{$node2}{set_nic}{'ifn-link2'} = "";
    foreach my $nic ( sort { $a cmp $b }
                      keys %{ $conf->{conf}{node}{$node1}{current_nic} } ) {
        my $mac = $conf->{conf}{node}{$node1}{current_nic}{$nic};
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; Checking node1: [$node1]'s: nic: [$nic], mac: [$mac].\n"
                           );
        if ( $mac eq $conf->{cgi}{anvil_node1_bcn_link1_mac} ) {
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; Unrecognized interface; node1: [$node1]: nic: [$nic], mac: [$mac].\n"
                               );
        }
    }
    foreach my $nic ( sort { $a cmp $b }
                      keys %{ $conf->{conf}{node}{$node2}{current_nic} } ) {
        my $mac = $conf->{conf}{node}{$node2}{current_nic}{$nic};
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; Checking node2: [$node2]'s: nic: [$nic], mac: [$mac].\n"
                           );
        if ( $mac eq $conf->{cgi}{anvil_node2_bcn_link1_mac} ) {
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; Unrecognized interface; node2: [$node2]: nic: [$nic], mac: [$mac].\n"
                               );
        }
    }
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; Set node1: [$node1]'s interfaces to; bcn-link1: [$conf->{conf}{node}{$node1}{set_nic}{'bcn-link1'}], bcn-link2: [$conf->{conf}{node}{$node1}{set_nic}{'bcn-link2'}], sn-link1: [$conf->{conf}{node}{$node1}{set_nic}{'sn-link1'}], sn-link2: [$conf->{conf}{node}{$node1}{set_nic}{'sn-link2'}], ifn-link1: [$conf->{conf}{node}{$node1}{set_nic}{'ifn-link1'}], ifn-link2: [$conf->{conf}{node}{$node1}{set_nic}{'ifn-link2'}].\n"
                       );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; Set node2: [$node2]'s interfaces to; bcn-link1: [$conf->{conf}{node}{$node2}{set_nic}{'bcn-link1'}], bcn-link2: [$conf->{conf}{node}{$node2}{set_nic}{'bcn-link2'}], sn-link1: [$conf->{conf}{node}{$node2}{set_nic}{'sn-link1'}], sn-link2: [$conf->{conf}{node}{$node2}{set_nic}{'sn-link2'}], ifn-link1: [$conf->{conf}{node}{$node2}{set_nic}{'ifn-link1'}], ifn-link2: [$conf->{conf}{node}{$node2}{set_nic}{'ifn-link2'}].\n"
                       );
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; cgi::remap_network: [$conf->{cgi}{remap_network}]\n"
                       );
    if ( $conf->{cgi}{remap_network} ) {
        $node1_class     = "highlight_note_bold";
        $node1_message   = "#!string!state_0032!#",
            $node2_class = "highlight_note_bold";
        $node2_message = "#!string!state_0032!#", $node1_remap_needed = 1;
        $node2_remap_needed = 1;
    }
...
sub map_network_on_node {
    ...
    if ($remap) {
        my $title =
            AN::Common::get_string( $conf,
                                    {  key       => "title_0174",
                                       variables => { node => $say_node, } } );
        print AN::Common::template( $conf,
                                    "install-manifest.html",
                                    "new-anvil-install-start-network-config",
                                    { title => $title, } );
    }
    my $return_code = 0;
    ...
    ###       under docroot and then wget from this machine.
    # First, make sure the script is downloaded and ready to run.
    my ( $error, $ssh_fh, $return ) = AN::Cluster::remote_call(
        $conf,
        {  node     => $node,
           port     => 22,
           user     => "root",
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; Failed: [$return_code]\n");
    ...
    if ( $conf->{node}{$node}{ssh_fh} !~ /^Net::SSH2/ ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; SSH File handle: [$conf->{node}{$node}{ssh_fh}] for node: [$node] doesn't exist, but it should. \n"
                           );
        $return_code = 8;
    }
    else {
        ...
        my $shell_call
            = "$conf->{path}{'anvil-map-network'} --script --summary";
        if ($remap) {
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; conf::node::${node}::current_nics::$nic: [$conf->{conf}{node}{$node}{current_nic}{$nic}].\n");
            ...
            while ( $stderr =~ s/^(.*)\n// ) {
                my $line = $1;
                AN::Cluster::record( $conf,
                            "$THIS_FILE " . __LINE__ . "; STDERR: [$line].\n" );
    ...
    if ($remap) {
        print AN::Common::template( $conf, "install-manifest.html",
                                    "new-anvil-install-end-network-config" );

        # We should now know this info.
        $conf->{conf}{node}{$node}{set_nic}{'bcn-link1'}
            = $conf->{conf}{node}{$node}{current_nic}{'bcn-link1'};
        $conf->{conf}{node}{$node}{set_nic}{'bcn-link2'}
            = $conf->{conf}{node}{$node}{current_nic}{'bcn-link2'};
        $conf->{conf}{node}{$node}{set_nic}{'sn-link1'}
            = $conf->{conf}{node}{$node}{current_nic}{'sn-link1'};
        $conf->{conf}{node}{$node}{set_nic}{'sn-link2'}
            = $conf->{conf}{node}{$node}{current_nic}{'sn-link2'};
        $conf->{conf}{node}{$node}{set_nic}{'ifn-link1'}
            = $conf->{conf}{node}{$node}{current_nic}{'ifn-link1'};
        $conf->{conf}{node}{$node}{set_nic}{'ifn-link2'}
            = $conf->{conf}{node}{$node}{current_nic}{'ifn-link2'};
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; node: [$node]: bcn-link1: [$conf->{conf}{node}{$node}{set_nic}{'bcn-link1'}], bcn-link2: [$conf->{conf}{node}{$node}{set_nic}{'bcn-link2'}], sn-link1: [$conf->{conf}{node}{$node}{set_nic}{'sn-link1'}], sn-link2: [$conf->{conf}{node}{$node}{set_nic}{'sn-link2'}], ifn-link1: [$conf->{conf}{node}{$node}{set_nic}{'ifn-link1'}], ifn-link2: [$conf->{conf}{node}{$node}{set_nic}{'ifn-link2'}]\n"
                           );
    }
...
sub install_programs {
    ...
    if ( not $ok ) {
        if (    ( not $conf->{node}{$node1}{internet} )
             || ( not $conf->{node}{$node2}{internet} ) ) {
            print AN::Common::template($conf,
                                       "install-manifest.html",
                                       "new-anvil-install-failed",
        ...
        elsif (    ( $conf->{node}{$node1}{os}{brand} =~ /Red Hat/ )
                || ( $conf->{node}{$node2}{os}{brand} =~ /Red Hat/ ) ) {
            print AN::Common::template($conf,
                                       "install-manifest.html",
                                       "new-anvil-install-failed",
...
sub install_missing_packages {
    ...
    my $to_install = "";
    foreach my $package ( sort { $a cmp $b }
                          keys %{ $conf->{packages}{to_install} } ) {

        # Watch for autovivication...
        if (    ( exists $conf->{node}{$node}{packages}{installed}{$package} )
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], package: [$package] already installed.\n");
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; node: [$node], package: [$package] needed.\n"
                               );
            $to_install .= "$package ";
        }
    }
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node: [$node], to_install: [$to_install]" );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; error: [$error], ssh_fh: [$ssh_fh], return: [$return (".@{$return}." lines)]\n");
        ...
        foreach my $line ( @{$return} ) {
            AN::Cluster::record( $conf,
                        "$THIS_FILE " . __LINE__ . "; return line: [$line]\n" );
        }
    }
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], package: [$package] installed.\n");
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; node: [$node], package: [$package] missing.\n"
                               );
            $missing .= "$package ";
        }
    }
    $missing =~ s/\s+$//;
    AN::Cluster::record( $conf,
          "$THIS_FILE " . __LINE__ . "; node: [$node], missing: [$missing]\n" );
        ...
        foreach my $line ( @{$return} ) {
            ...
            if ( $line eq "failed" ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; Failed to delete the 'virbr0' bridge.\n"
                                   );
            }
...
sub add_an_repo {
    ...
    if ( $node1_rc eq "2" ) {
        ...
        $node1_class = "highlight_warning_bold";
        $node1_message = "#!string!state_0018!#", $ok = 0;
    }
    if ( $node2_rc eq "2" ) {
        ...
        $node2_class = "highlight_warning_bold";
        $node2_message = "#!string!state_0018!#", $ok = 0;
    }
...
sub add_an_repo_to_node {
    ...
    my ( $error, $ssh_fh, $return )
        ...
        {  node     => $node,
           ...
           'close' => 0,
           shell_call =>
               "if [ -e '/etc/yum.repos.d/an.repo' ]; then echo 1; else curl --silent https://alteeve.ca/repo/el6/an.repo --output /etc/yum.repos.d/an.repo; if [ -e '/etc/yum.repos.d/an.repo' ]; then yum clean all --quiet; echo 2; else echo 9; fi; fi",
        } );
...
sub update_nodes {
    ...
    update_node( $conf,
                 $conf->{cgi}{anvil_node2_current_ip},
                 $conf->{cgi}{anvi2_node1_current_password} );
...
sub update_node {
    ...
    AN::Cluster::record(
                ...
                "$THIS_FILE "
                    . __LINE__
                    . "; error: [$error], ssh_fh: [$ssh_fh], return: [$return ("
                    . @{$return}
                    . " lines)]\n" );
    $conf->{node}{$node}{internet} = 0;
    foreach my $line ( @{$return} ) {
...
sub verify_internet_access {
    ...
    if ( not $node1_online ) {
        $node1_class = "highlight_warning_bold";
        $node1_message = "#!string!state_0021!#", $ok = 0;
    }
    if ( not $node2_online ) {
        $node2_class = "highlight_warning_bold";
        $node2_message = "#!string!state_0021!#", $ok = 0;
    }
    print AN::Common::template( $conf,
                                "install-manifest.html",
                                "new-anvil-install-message",
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], pings_sent: [$pings_sent], pings_received: [$pings_received]\n");
    ...
    AN::Cluster::record( $conf,
                    "$THIS_FILE " . __LINE__ . "; node: [$node], ok: [$ok]\n" );
    return ($ok);
}
...
sub verify_matching_free_space {
    ...
    my $ok = 1;
    my ( $node1_use_device, $node1_free_space )
        = get_partition_data( $conf,
                              $conf->{cgi}{anvil_node1_current_ip},
                              $conf->{cgi}{anvil_node1_current_password} );
    my ( $node2_use_device, $node2_free_space )
        = get_partition_data( $conf,
                              $conf->{cgi}{anvil_node2_current_ip},
                              $conf->{cgi}{anvil_node2_current_password} );
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node1_free_space: [$node1_free_space], node2_free_space: [$node2_free_space]\n"
                       );

    # Message stuff
    my $node1_class   = "highlight_good_bold";
    my $node1_message = "$node1_use_device:"
        . AN::Cluster::bytes_to_hr( $conf, $node1_free_space );
    my $node2_class   = "highlight_good_bold";
    my $node2_message = "$node2_use_device:"
        . AN::Cluster::bytes_to_hr( $conf, $node2_free_space );
    my $message = "";
    ...
    my $media_library_byte_size =
        AN::Cluster::hr_to_bytes( $conf, $media_library_size,
                                  $media_library_unit, 1 );
    my $minimum_space_needed = $media_library_byte_size;
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; media_library_byte_size: [$media_library_byte_size], minimum_space_needed: [$minimum_space_needed]\n"
                       );
    ...
    if ( $storage_pool1_unit eq "%" ) {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; minimum_space_needed: [$minimum_space_needed]\n"
                           );
    }
    else {
        $storage_pool1_byte_size =
            AN::Cluster::hr_to_bytes( $conf, $storage_pool1_size,
                                      $storage_pool1_unit, 1 );
        $minimum_space_needed += $storage_pool1_byte_size;
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; storage_pool1_byte_size: [$storage_pool1_byte_size], minimum_space_needed: [$minimum_space_needed]\n"
                           );
    }
    ...
    if ($ok) {
        ...
        if ( $storage_pool1_unit eq "%" ) {
            ...
            AN::Cluster::record( $conf,
                         "$THIS_FILE " . __LINE__ . "; percent: [$percent]\n" );
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; >> pool1_byte_size: [$pool1_byte_size]\n"
                               );
            ...
            if ($pool1_difference) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; pool1_difference: [$pool1_difference]\n"
                                   );
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; >> pool2_byte_size: [$pool2_byte_size]\n"
                               );
            if ( $pool2_byte_size < 0 ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; pool2_difference: [$pool1_difference]\n"
                                   );
                if ($pool2_difference) {
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; media_library_byte_size: [$media_library_byte_size] + pool1_byte_size: [$pool1_byte_size] + pool2_byte_size: [$pool2_byte_size]\n"
                               );
            my $total_allocated
                = (   $media_library_byte_size
                    + $pool1_byte_size
                    + $pool2_byte_size );
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; total_allocated: [$total_allocated], total_free_space: [$total_free_space]\n"
                               );
            if ( $total_allocated > $total_free_space ) {
                my $too_much = $total_allocated - $total_free_space;
                AN::Cluster::record( $conf,
                       "$THIS_FILE " . __LINE__ . "; too_much: [$too_much]\n" );
            ...
            AN::Cluster::record( $conf,
                                 "$THIS_FILE "
                                     . __LINE__
                                     . "; cgi::anvil_storage_pool1_byte_size: [$conf->{cgi}{anvil_storage_pool1_byte_size}], cgi::anvil_storage_pool2_byte_size: [$conf->{cgi}{anvil_storage_pool2_byte_size}]\n"
                               );
        }
        else {
        ...
        AN::Cluster::record( $conf,
                             "$THIS_FILE "
                                 . __LINE__
                                 . "; cgi::anvil_media_library_byte_size: [$conf->{cgi}{anvil_media_library_byte_size}]\n"
                           );
    }
    else {
        print AN::Common::template( $conf,
    ...
    my $say_media_library = AN::Cluster::bytes_to_hr( $conf,
                                  $conf->{cgi}{anvil_media_library_byte_size} );
    my $say_pool1 = AN::Cluster::bytes_to_hr( $conf,
                                  $conf->{cgi}{anvil_storage_pool1_byte_size} );
    my $say_pool2 = AN::Cluster::bytes_to_hr( $conf,
                                  $conf->{cgi}{anvil_storage_pool2_byte_size} );
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; cgi::anvil_media_library_byte_size: [$conf->{cgi}{anvil_media_library_byte_size} ($say_media_library)], cgi::anvil_storage_pool1_byte_size: [$conf->{cgi}{anvil_storage_pool1_byte_size} ($say_pool1)], cgi::anvil_storage_pool2_byte_size: [$conf->{cgi}{anvil_storage_pool2_byte_size} ($say_pool2)]\n"
                       );
    ...
    $conf->{cgi}{anvil_storage_partition_2_byte_size}
        = $conf->{cgi}{anvil_storage_pool2_byte_size};
    my $say_partition_1 = AN::Cluster::bytes_to_hr( $conf,
                            $conf->{cgi}{anvil_storage_partition_1_byte_size} );
    my $say_partition_2 = AN::Cluster::bytes_to_hr( $conf,
                            $conf->{cgi}{anvil_storage_partition_2_byte_size} );
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; cgi::anvil_storage_partition_1_byte_size: [$conf->{cgi}{anvil_storage_partition_1_byte_size} ($say_partition_1)], cgi::anvil_storage_partition_2_byte_size: [$conf->{cgi}{anvil_storage_partition_2_byte_size} ($say_partition_2)]\n"
                       );
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], disk: [$disk]\n");
        my $shell_call
            = "if [ ! -e /sbin/parted ]; then yum --quiet -y install parted; echo parted installed; fi && parted /dev/$disk unit B print free";
        if ( not $conf->{node}{$node}{internet} ) {
            $shell_call
                = "if [ ! -e /sbin/parted ]; then echo parted not installed; else parted /dev/$disk unit B print free; fi";
        }
        my ( $error, $ssh_fh, $return )
            = AN::Cluster::remote_call($conf,
                                       { node     => $node,
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], disk: [$disk], line: [$line]\n");
            if ( $line eq "parted not installed" ) {
                ...
                AN::Cluster::record( $conf,
                                     "$THIS_FILE "
                                         . __LINE__
                                         . "; node: [$node], Installed 'parted' RPM.\n"
                                   );
            }
       ...
       #              part  start end   size  type  - don't care about the rest.
                ...
                my $partition_size   = $4;
                my $partition_type   = $5;
                $conf->{node}{$node}{disk}{$disk}{partition}{$partition_number}
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], disk: [$disk], partition: [$partition_number], start: [$conf->{node}{$node}{disk}{$disk}{partition}{$partition_number}{start}], end: [$conf->{node}{$node}{disk}{$disk}{partition}{$partition_number}{end}], size: [$conf->{node}{$node}{disk}{$disk}{partition}{$partition_number}{size}], type: [$conf->{node}{$node}{disk}{$disk}{partition}{$partition_number}{type}]\n");
                ...
                $conf->{node}{$node}{disk}{$disk}{free_space}{start}
                    = $free_space_start;
                $conf->{node}{$node}{disk}{$disk}{free_space}{end}
                    = $free_space_end;
                $conf->{node}{$node}{disk}{$disk}{free_space}{size}
                    = $free_space_size;
...
#AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; node: [$node], disk: [$disk], Yes; device: [$device], free_space_size: [$free_space_size]\n");
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node: [$node], device: [$device], largest_free_space: [$largest_free_space]\n"
                       );
...
sub verify_node_is_not_in_a_cluster {
    ...
    my $ok = 1;
    my ($node1_cluster_conf)
        = read_cluster_conf( $conf,
                             $conf->{cgi}{anvil_node1_current_ip},
                             $conf->{cgi}{anvil_node1_current_password} );
    my ($node2_cluster_conf)
        = read_cluster_conf( $conf,
                             $conf->{cgi}{anvil_node2_current_ip},
                             $conf->{cgi}{anvil_node2_current_password} );
    ...
    my $node1_class = "highlight_good_bold";
    my $node1_message
        = AN::Common::get_string( $conf, { key => "state_0019" } );
    my $node2_class = "highlight_good_bold";
    my $node2_message
        = AN::Common::get_string( $conf, { key => "state_0019" } );
    if ($node1_cluster_conf) {
        $node1_class = "highlight_bad_bold";
        $node1_message
            = AN::Common::get_string( $conf, { key => "state_0020" } );
        $ok = 0;
    }
...
sub verify_os {
    ...
    my $ok = 1;
    my ( $node1_major_version, $node1_minor_version )
        = get_node_os_version( $conf,
                               $conf->{cgi}{anvil_node1_current_ip},
                               $conf->{cgi}{anvil_node1_current_password} );
    my ( $node2_major_version, $node2_minor_version )
        = get_node_os_version( $conf,
                               $conf->{cgi}{anvil_node2_current_ip},
                               $conf->{cgi}{anvil_node2_current_password} );
    my $node1 = $conf->{cgi}{anvil_node1_current_ip};
...
sub check_connection {
    ...
    AN::Cluster::record( $conf,
                         "$THIS_FILE "
                             . __LINE__
                             . "; node1_access: [$node1_access], node2_access: [$node2_access]\n"
                       );

    my $node1_class = "highlight_good_bold";
    my $node1_message
        = AN::Common::get_string( $conf, { key => "state_0017" } );
    my $node2_class = "highlight_good_bold";
    my $node2_message
        = AN::Common::get_string( $conf, { key => "state_0017" } );
    if ( not $node1_access ) {
        $node1_class = "highlight_bad_bold";
        $node1_message
            = AN::Common::get_string( $conf, { key => "state_0018" } );
    }
    if ( not $node2_access ) {
        $node2_class = "highlight_bad_bold";
        $node2_message
...
#foreach my $line (@{$return}) { AN::Cluster::record($conf, "$THIS_FILE ".__LINE__."; return line: [$line]\n"); }
    ...
    AN::Cluster::record( $conf,
            "$THIS_FILE " . __LINE__ . "; node: [$node], access: [$access]\n" );
