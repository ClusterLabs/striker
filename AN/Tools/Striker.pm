...
sub update_striker_conf
{
	...
	my $parameter = shift;
	my $an        = $self->parent;
	$an->Log->entry({log_level => 3, title_key => "tools_log_0001", title_variables => { function => "update_striker_conf" }, message_key => "tools_log_0002", file => $THIS_FILE, line => __LINE__});
...
sub _add_server_to_anvil
{
	...
	if (not $skip_scan)
	{
		...
		$an->Log->entry({log_level => 3, message_key => "log_0231", file => $THIS_FILE, line => __LINE__});
...
sub _cold_stop_anvil
{
	...
	if (($an->data->{sys}{anvil}{node1}{online}) or ($an->data->{sys}{anvil}{node2}{online}))
	{
		...
		# Now stop the node(s).
		my $stop_order = ["node2", "node1"];
		if (($an->data->{sys}{anvil}{node1}{online}) && ($an->data->{sys}{anvil}{node2}{online}))
		{
			# Both are up, so make sure I want to still stop node 2 first.
			...
			### NOTE: We know both nodes are on if we're here, so we don't check for other 'none'
			###       types.
			...
			# If 'none:both_drbdsource', abort.
			if ($first_off eq "none:both_drbdsource")
			{
				# Wow... really?
				my $say_title   = $an->String->get({key => "title_0184"});
				my $say_message = $an->String->get({key => "message_0508", variables => { anvil => $anvil_name }});
				print $an->Web->template({file => "server.html", template => "cold-stop-aborted", replace => { 
					title	=>	$say_title,
					message	=>	$say_message,
				}});
			...
			elsif ($first_off =~ /^node1/)
			{
				$stop_order = ["node1", "node2"];
			}
		}
		elsif ($an->data->{sys}{anvil}{node1}{online})
		{
			# Only node 1 is up
			$stop_order = ["node1"];
		}
		...
		# Now loop through the node(s)
		foreach my $node_key (@{$stop_order})
		{
			my $node_name = $an->data->{sys}{anvil}{$node_key}{name};
			my $target    = $an->data->{sys}{anvil}{$node_key}{use_ip};
			my $port      = $an->data->{sys}{anvil}{$node_key}{use_port};
			...
			my $shell_output = $an->String->get({key => "message_0510", variables => { node_name => $node_name }})."<br />\n";;
			...
			foreach my $line (@{$return})
			{
				...
				if ($line =~ /poweroff: (.*)$/)
				{
					...
					my $node_uuid     = $an->data->{sys}{anvil}{$node_key}{uuid};
					$an->Log->entry({log_level => 3, message_key => "an_variables_0003", message_variables => {
						name1 => "shutdown_name", value1 => $shutdown_name,
						name2 => "node_name",     value2 => $node_name,
						name3 => "node_uuid",     value3 => $node_uuid,
					}, file => $THIS_FILE, line => __LINE__});
					if (not $node_uuid)
					...
					# Add some output
					$shell_output .= $an->String->get({key => "message_0509", variables => { node_name => $node_name }})."<br />\n";
				}
				elsif ($line =~ /rc:(\d+)/)
				{
				...
				{
					# Just show the user the raw string
					$shell_output .= $an->Web->parse_text_line({line => $line})."<br />\n";
				}
			}
...
sub _parse_cluster_conf
{
	...
	foreach my $line (@{$data})
	{
		...
		if (($in_method) && ($line =~ /<device\s/))
		{
			...
			$an->data->{node}{$this_node}{fence}{method}{$in_method}{device}{$device_count}{lanplus}         = $lanplus;
			...
			$an->Log->entry({log_level => 3, message_key => "an_variables_0007", message_variables => {
				...
				name6 => "node::${this_node}::fence::method::${in_method}::device::${device_count}::lanplus", value6 => $an->data->{node}{$this_node}{fence}{method}{$in_method}{device}{$device_count}{lanplus},
				name7 => "password_script",                                                                   value7 => $password_script,
			}, file => $THIS_FILE, line => __LINE__});
			$an->Log->entry({log_level => 4, message_key => "an_variables_0001", message_variables => {
		...
		if ($in_fence_device)
		{
			...
			$an->data->{fence}{$name}{lanplus}         = $lanplus;
			...
			$an->Log->entry({log_level => 3, message_key => "an_variables_0006", message_variables => {
				...
				name5 => "lanplus",   value5 => $an->data->{fence}{$name}{lanplus},
				name6 => "action",    value6 => $an->data->{fence}{$name}{action},
			}, file => $THIS_FILE, line => __LINE__});
			$an->Log->entry({log_level => 4, message_key => "an_variables_0002", message_variables => {
				name1 => "password",        value1 => $an->data->{fence}{$name}{password},
				name2 => "password_script", value2 => $an->data->{fence}{$name}{password_script},
			}, file => $THIS_FILE, line => __LINE__});
		}
			...
			foreach my $device_count (sort {$a cmp $b} keys %{$an->data->{node}{$this_node}{fence}{method}{$in_method}{device}})
			{
				...
				foreach my $name (sort {$a cmp $b} keys %{$an->data->{fence}})
				{
					if ($name eq $an->data->{node}{$this_node}{fence}{method}{$in_method}{device}{$device_count}{name})
					{
						...
						my $lanplus         = $an->data->{fence}{$name}{lanplus};
						...
						if ($an->data->{node}{$this_node}{fence}{method}{$in_method}{device}{$device_count}{lanplus})
						{
							$lanplus = $an->data->{node}{$this_node}{fence}{method}{$in_method}{device}{$device_count}{lanplus};
						}
...
sub _provision_server
{
	...
	$an->data->{cgi}{os_variant} =~ s/2O19/2019/;
