...
sub add_rsa_key_to_target
{
	...
	$an->Log->entry({log_level => 2, message_key => "an_variables_0006", message_variables => {
		...
		name5 => "users_home",      value5 => $users_home, 
		name6 => "authorized_keys", value6 => $authorized_keys,
	...
	foreach my $line (@{$return})
	{
		...
		if ($line =~ /rc:(\d+)$/)
		{
			...
			{
				# Add it.
				my $shell_call = $an->data->{path}{ssh}." $user\@$target \"".$an->data->{path}{'echo'}." 'ssh-rsa $key ' >> $authorized_keys; ".$an->data->{path}{'grep'}." -q 'ssh-rsa $key ' $authorized_keys; ".$an->data->{path}{echo}." rc:\\\$?";
				$an->Log->entry({log_level => 2, message_key => "an_variables_0001", message_variables => {
					name1 => "shell_call", value1 => $shell_call, 
				}, file => $THIS_FILE, line => __LINE__});
				my ($error, $ssh_fh, $return) = $an->Remote->remote_call({
...
sub generate_rsa_public_key
{
	...
	$an->Log->entry({log_level => 2, message_key => "notice_message_0006", file => $THIS_FILE, line => __LINE__});
	my $shell_call  = "
...
";
	...
	if (-e $rsa_private_file)
	{
		# Yup!
		$ok = 1;
		$an->Log->entry({log_level => 2, message_key => "notice_message_0007", message_variables => {
			user => $user, 
		}, file => $THIS_FILE, line => __LINE__});
	}
	else
...
sub _check_known_hosts_for_target
{
	...
	$an->Log->entry({log_level => 2, title_key => "tools_log_0001", title_variables => { function => "_check_known_hosts_for_target" }, message_key => "an_variables_0004", message_variables => { 
		name1 => "target",      value1 => $target,
...
sub _call_ssh_keyscan
{
	...
	my $target      = $parameter->{target};
	my $known_hosts = $parameter->{known_hosts};
	$an->Log->entry({log_level => 2, title_key => "tools_log_0001", title_variables => { function => "_call_ssh_keyscan" }, message_key => "an_variables_0003", message_variables => { 
		name1 => "user",        value1 => $user,
		name2 => "target",      value2 => $target,
