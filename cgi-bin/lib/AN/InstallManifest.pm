...
sub configure_network_on_node
{
	...
	if (1)
	{
		$iptables =  "# Generated by: [$THIS_FILE] on: [".AN::Cluster::get_date($conf)."].\n";
		$iptables .= "*filter\n";
...
=pod
		...
		$iptables .= "-A INPUT -s $conf->{cgi}{anvil_bcn_network}/$conf->{cgi}{anvil_bcn_subnet} -d $conf->{cgi}{anvil_bcn_network}/$conf->{cgi}{anvil_bcn_subnet} -p tcp -m conntrack --ctstate NEW -m multiport -m tcp --dports 123,5404,5405,5800,5900:$vnc_range,11111,16851,21064,49152:49216 -j ACCEPT \n";
		...
		$iptables .= "-A INPUT -s $conf->{cgi}{anvil_sn_network}/$conf->{cgi}{anvil_sn_subnet} -d $conf->{cgi}{anvil_sn_network}/$conf->{cgi}{anvil_sn_subnet} -p tcp -m conntrack --ctstate NEW -m multiport -m tcp --dports 5404,5405,7788,7789,11111,16851,21064 -j ACCEPT \n";
		...
		### IFN rules
		$iptables .= "-A INPUT -s $conf->{cgi}{anvil_ifn_network}/$conf->{cgi}{anvil_ifn_subnet} -d $conf->{cgi}{anvil_ifn_network}/$conf->{cgi}{anvil_ifn_subnet} -p tcp -m conntrack --ctstate NEW -m multiport -m tcp --dports 123,5800,5900:$vnc_range,49152:49216 -j ACCEPT \n";
