...
sub copy_file
{
	...
	my $output = "";
	my $sc     = "$conf->{path}{cp} -f $source $destination";
...
sub load_install_manifest
{
	...
	if (-e $manifest_file)
	{
		...
		{
			#print "Node: [$node] -> [$data->{node}{$node}]\n";
			#print Dumper $data->{node}{$node};
			foreach my $a (keys %{$data->{node}{$node}})
			{
				if ($a eq "ipmi")
				{
					...
					my $on       = $data->{node}{$node}{$a}->[0]->{address}->[0]->{on};
					...
					$conf->{install_manifest}{$file}{node}{$node}{ipmi}{on}       = $on;
				...
				{
					#print "a: [$a] -> [$data->{node}{$node}{pdu}->[0]->{on}]\n";
					#print Dumper $data->{node}{$node}{pdu}->[0]->{on};
					foreach my $b (keys @{$data->{node}{$node}{pdu}->[0]->{on}})
					{
						my $name     = $data->{node}{$node}{pdu}->[0]->{on}->[$b]->{name};
						my $port     = $data->{node}{$node}{pdu}->[0]->{on}->[$b]->{port};
						my $agent    = $data->{node}{$node}{pdu}->[0]->{on}->[$b]->{agent};
						my $user     = $data->{node}{$node}{pdu}->[0]->{on}->[$b]->{user};
						my $password = $data->{node}{$node}{pdu}->[0]->{on}->[$b]->{password};
						...
						#print Dumper $data->{node}{$node}{pdu}->[0]->{on}->[$b];
					}
				}
				else
				{
					#print "a: [$a] -> [$data->{node}{$node}{$a}]\n";
					foreach my $b (keys @{$data->{node}{$node}{$a}})
					{
		...
		foreach my $a (@{$data->{common}})
		{
			foreach my $b (keys %{$a})
			{
				...
				{
					### wtf; why does this need 'keys @{}' ??
					foreach my $c (keys @{$a->{$b}->[0]->{daemon}})
					{
						my $name          = $a->{$b}->[0]->{daemon}->[$c]->{name};
						my $start_on_boot = $a->{$b}->[0]->{daemon}->[$c]->{start_on_boot};
						$conf->{install_manifest}{$file}{common}{daemon}{$name}{start_on_boot} = $start_on_boot;
					}
				}
				elsif ($b eq "file")
				{
					...
					foreach my $c (keys @{$a->{$b}})
					{
						my $name    = $a->{$b}->[$c]->{name};
						my $mode    = $a->{$b}->[$c]->{mode};
						my $owner   = $a->{$b}->[$c]->{owner};
						my $group   = $a->{$b}->[$c]->{group};
						my $content = $a->{$b}->[$c]->{content};
				...
				{
					foreach my $c (keys @{$a->{$b}->[0]->{rule}})
					{
						my $sequence = $a->{$b}->[0]->{rule}->[$c]->{sequence};
						my $rule     = $a->{$b}->[0]->{rule}->[$c]->{rule};
						if (exists $conf->{install_manifest}{$file}{common}{iptables}{rule}{$sequence})
						{
							die "Duplicate 'iptables' rule sequence in install manifest: [$file]\n"; 
						}
						$conf->{install_manifest}{$file}{common}{iptables}{rule}{$sequence}{rule} = $rule;
					}
				}
				...
				elsif ($b eq "networks")
				{
					foreach my $c (keys %{$a->{$b}->[0]})
					{
						...
						{
							foreach my $d (keys @{$a->{$b}->[0]->{$c}->[0]->{bridge}})
							{
								my $name = $a->{$b}->[0]->{$c}->[0]->{bridge}->[$d]->{name};
								my $on   = $a->{$b}->[0]->{$c}->[0]->{bridge}->[$d]->{on};
				...
				{
					foreach my $c (keys @{$a->{$b}->[0]->{pdu}})
					{
						my $name = $a->{$b}->[0]->{pdu}->[$c]->{name};
						my $ip   = $a->{$b}->[0]->{pdu}->[$c]->{ip};
						my $on   = $a->{$b}->[0]->{pdu}->[$c]->{on};
						
						$conf->{install_manifest}{$file}{common}{pdu}{$name}{ip} = $ip;
						$conf->{install_manifest}{$file}{common}{pdu}{$name}{on} = $on;
				...
				{
					foreach my $c (keys @{$a->{$b}->[0]->{striker}})
					{
						my $name   = $a->{$b}->[0]->{striker}->[$c]->{name};
						my $bcn_ip = $a->{$b}->[0]->{striker}->[$c]->{bcn_ip};
						my $ifn_ip = $a->{$b}->[0]->{striker}->[$c]->{ifn_ip};
				...
				{
					foreach my $c (keys @{$a->{$b}->[0]->{switch}})
					{
						my $name = $a->{$b}->[0]->{switch}->[$c]->{name};
						my $ip   = $a->{$b}->[0]->{switch}->[$c]->{ip};
						my $on   = $a->{$b}->[0]->{switch}->[$c]->{on};
						$conf->{install_manifest}{$file}{common}{switch}{$name}{ip} = $ip;
						$conf->{install_manifest}{$file}{common}{switch}{$name}{on} = $on;
				...
				{
					foreach my $c (keys @{$a->{$b}->[0]->{ups}})
					{
						my $name = $a->{$b}->[0]->{ups}->[$c]->{name};
						my $ip   = $a->{$b}->[0]->{ups}->[$c]->{ip};
						my $on   = $a->{$b}->[0]->{ups}->[$c]->{on};
						my $type = $a->{$b}->[0]->{ups}->[$c]->{type};
						my $port = $a->{$b}->[0]->{ups}->[$c]->{port};
						$conf->{install_manifest}{$file}{common}{ups}{$name}{ip}   = $ip;
						$conf->{install_manifest}{$file}{common}{ups}{$name}{on}   = $on;
...
";
	...
	{
		print "			<switch name=\"$conf->{cgi}{anvil_switch2_name}\" ip=\"$conf->{cgi}{anvil_switch2_ip}\" on=\"bcn\" />";
	}
	$xml .="
		</switch>
		<ups>
...
sub check_node_status
{
	...
	foreach my $node (sort {$a cmp $b} @{$conf->{clusters}{$cluster}{nodes}})
	{
		...
		push @{$conf->{online_nodes}}, $node
		    if AN::Cdb::check_node_daemons($conf, $node);
	}
...
sub gather_node_details
{
	...
	if ($conf->{node}{$node}{connected})
	{
		...
		# Get drbd info
		($error, $ssh_fh, my $drbd_status) = remote_call($conf, {
			node		=>	$node,
			port		=>	$conf->{node}{$node}{port},
			user		=>	"root",
			password	=>	$conf->{'system'}{root_password},
			ssh_fh		=>	$ssh_fh,
			'close'		=>	0,
			shell_call	=>	"drbdadm status",
		});
		#record($conf, "$THIS_FILE ".__LINE__."; error: [$error], ssh_fh: [$ssh_fh], drbd_status: [$drbd_status (".@{$drbd_status}." lines)]\n");
		($error, $ssh_fh, my $drbd_res_file) = remote_call($conf, {
			node		=>	$node,
			port		=>	$conf->{node}{$node}{port},
			user		=>	"root",
			password	=>	$conf->{'system'}{root_password},
			ssh_fh		=>	$ssh_fh,
			'close'		=>	0,
			shell_call	=>	"cat /etc/drbd.d/r?.res",
		});
		#record($conf, "$THIS_FILE ".__LINE__."; error: [$error], ssh_fh: [$ssh_fh], drbd_res_file: [$drbd_res_file (".@{$drbd_res_file}." lines)]\n");
...
sub parse_cluster_conf
{
	...
	foreach my $line (@{$array})
	{
		...
		if ($line =~ /<clusternode.*?name="(.*?)"/)
		{
			...
			{
				$this_node = AN::Cdb::get_peer_node($conf, $node);
	...
	my $peer = AN::Cdb::get_peer_node($conf, $node);
